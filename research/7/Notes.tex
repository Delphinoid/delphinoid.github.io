...
\noindent\begin{remark} This definition is slightly unusual. It is more typical to talk of {\em directed order ideals} of partially ordered sets $(P, \leq)$, which are non-empty subsets that are lower sets and upward directed sets. The reason why we have chosen to use coideals rather than ideals is due to how we have defined our partial order; the standard notion of a directed order ideal goes ``downwards'', while we want to go ``upwards''. The reasoning behind this somewhat quirky setup will hopefully become clear once we look at some explicit examples of Jordan--H\"{o}lder filtrations.\\
...
\end{split}
\end{align*}
\noindent Therefore, let $T \coloneqq G^\theta$. The corresponding Weyl group is $W \coloneqq N_G(T)/T = S_2 = \{1, -1\}$, since
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent similarly, $T_K \coloneqq T$ is obviously then a maximal torus in $K$, giving us $W_K \coloneqq N_K(T_K)/T_K = \{1\}$. The only choice of simple roots we can make is $S = \{-1\}$, whence $R = \mathbb{R}[\alpha_{-1}]$. The action of $W$ on $R$ is given by $1 : r \mapsto r$ and $-1 : r \mapsto -r$, for $r \in R$, giving us $R^{W_K} = R$ and an easy $(R^{W_K}, R)$-bimodule structure for $R$. This gives us a very explicit right module category over $\mathbb{S}\textcat{Bim}(W, S)$! A standard exercise we can now do is to compute the Jordan--H\"{o}lder filtrations of $\mathcal{M}_{LV}^0$. \textcolor{red}{When $T = G^\theta$, don't we always have that $W^\theta = \{e\}$? Using $W/W_K$ makes much more sense, so why is $W^\theta$ written in the most general setting?}\\[-1.5\baselineskip]%; therefore it is easily seen to be multifinitary.\\[-1.5\baselineskip]
\begin{center}
\rule{0.5\linewidth}{1pt}
\end{center}
\noindent\\[-\baselineskip]
...
\begin{alignat*}{2}
R_1 \otimes_R B_1 \cong (R \otimes_R R) \otimes_R R(1) &\cong R,&\qquad R_1 \otimes_R B_{-1} \cong (R \otimes_R R) \otimes_{R^{-1}} R(1) &\cong B_{-1},\\
R_{-1} \otimes_R B_1 \cong (R_{-1} \otimes_R R) \otimes_R R(1) &\cong R_{-1},&\qquad R_{-1} \otimes_R B_{-1} \cong (R_{-1} \otimes_R R) \otimes_{R^{-1}} R(1) &\cong B_{-1},
\end{alignat*}
\noindent where we note that $R_1 \cong R \cong B_1$. Thus our three indecomposables are $R$, $R_{-1}$ and $B_{-1}$. From this we obtain the two complete filtrations
\begin{align*}
\begin{split}
\{B_{-1}\} \subset \{R_{-1}, B_{-1}\} \subset \{R, R_{-1}, B_{-1}\}\qquad\text{and}\qquad\{B_{-1}\} \subset \{R, B_{-1}\} \subset \{R, R_{-1}, B_{-1}\}.
\end{split}
\end{align*}
\newpage
\noindent It is immediately obvious that the corresponding weak composition quotients will be equivalent, so we will just compute them for the filtration given by $Q_1 \coloneqq \{B_{-1}\}$, $Q_2 \coloneqq \{R_{-1}, B_{-1}\}$ and $Q_3 \coloneqq \{R, R_{-1}, B_{-1}\}$. Defining subcategories $\mathcal{M}_{Q_i} \coloneqq \langle q \otimes_R X : q \in Q_i, X \in \obset(\mathbb{S}\textcat{Bim}(W, S))\rangle_{\oplus,\ominus,(1)}$ and two-sided ideals $\mathcal{I}_{Q_i}$ of $\mathcal{M}_{LV}^0$, we are able to define the quotient subcategories
\begin{align*}
\begin{split}
\mathcal{M}_{Q_3/Q_2} \coloneqq \mathcal{M}_{Q_3}/\mathcal{I}_{Q_2} = \langle R\hphantom{{}_{-1}} \otimes_R X : X \in \obset(\mathbb{S}\textcat{Bim}(W, S))\rangle_{\oplus,\ominus,(1)},\\
\mathcal{M}_{Q_2/Q_1} \coloneqq \mathcal{M}_{Q_2}/\mathcal{I}_{Q_1} = \langle R_{-1} \otimes_R X : X \in \obset(\mathbb{S}\textcat{Bim}(W, S))\rangle_{\oplus,\ominus,(1)},\\
\mathcal{M}_{Q_1/Q_0} \coloneqq \mathcal{M}_{Q_1}/\mathcal{I}_{Q_0} = \langle B_{-1} \otimes_R X : X \in \obset(\mathbb{S}\textcat{Bim}(W, S))\rangle_{\oplus,\ominus,(1)}.
\end{split}
\end{align*}
