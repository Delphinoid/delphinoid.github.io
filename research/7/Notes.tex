...
\end{split}
\end{align*}
\noindent But this system has no solutions, and hence $R^t \otimes_{R^{t,u}} R$ is not an indecomposable $(R^{s,t}, R)$-bimodule! In fact, through similar reasoning, we find that $R^t \otimes_{R^{s,t}} R$, $R^s \otimes_{R^{s,t}} R$ and $R^s \otimes_{R^{t,u}} R$ are also not indecomposable $(R^{s,t}, R)$-bimodules (and of course we cannot have any subset of $R^u$ on the left, as this is zero as a left $R^{s,t}$-module). \textcolor{red}{We will likely need some deeper insight from $\mathbb{S}\textcat{Bim}(A_3)$ to find the remaining two indecomposables.}
\newpage
...
\noindent To-do: determine the $G_2$ ``$W$-graph'' and check if it looks different to the decategorified $W$-graph \textcolor{red}{(to do this, we may first have to find the analogue of standard filtrations and prove the validity of Anna's method -- I think this should be the next step)}; check if $G_2$ has non-cell simple transitive module subcategories; determine the indecomposables in type $A_n$.\\\\
