...
\noindent Observe that a monoidal category $\mathcal{C}$ is (multi)finitary if and only if its monoidal delooping $\textcat{B}\mathcal{C}$ is (multi)finitary.\\
...
\end{center}
\noindent\\[-\baselineskip]
\noindent As before, $\mathcal{N}_{LV}^0(\textnormal{SU}(2, 1))$ will be generated by the standard bimodules $R_w$, where $w$ is a coset representative for $W_K\backslash W = \{W_K, W_Kt, W_Kts\}$. We therefore know automatically that $R$, $R_t$ and $R_{ts}$ will be indecomposable; in particular, it follows from \cite[Lemma 6.2.3]{LR22} that $R_s \cong R_{ss} = R$, $R_{st} \cong R_{sst} = R_t$ and $R_{tst} \cong R_{stst} = R_{ts}$. The remaining candidates for the indecomposables of $\mathcal{N}_{LV}^0(\textnormal{SU}(2, 1))$ are $B_s$, $B_t$, $R_t \otimes_R B_s$, $R_t \otimes_R B_t$, $R_{ts} \otimes_R B_s$ and $R_{ts} \otimes_R B_t$. By our previous reasoning from the $\mathcal{N}_{LV}^0(\textnormal{SL}(2, \mathbb{R}))$ example, $R_t \otimes_R B_t \cong B_t$ and $R_{ts} \otimes_R B_s \cong R_t \otimes_R B_s$. %We claim that all of these objects are indecomposable except for $R_{ts} \otimes_R B_t$.
\textcolor{red}{Supposedly $B_s$ and $R_{ts} \otimes_R B_t$ are decomposable, with our last indecomposable being ``$R^s \otimes_{R^W} R$''. This doesn't make sense to me, and seems to contradict the fact that $R$, $R_t$ and $R_{ts}$ generate $\mathcal{N}_{LV}^0(\textnormal{SU}(2, 1))$. Maybe I should start by showing that $B_t$ and $R_t \otimes_R B_s$ are decomposable.}
\end{example}
\newpage
