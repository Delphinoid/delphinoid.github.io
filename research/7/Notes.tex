...
\end{tikzpicture}.
\end{center}
\noindent\\ This picture becomes much more insightful in light of \hyperref[ActionPreorder]{Lemma \ref*{ActionPreorder}} and \hyperref[Transitive2]{Proposition \ref*{Transitive2}}. \textcolor{red}{Actually this picture is kind of just dumb and confusing, a concrete example would be better.}\\
...
\noindent A statement of and reference for these results can be found in \cite[Theorem 5.24]{EMTW20}, as well as an algorithm for finding the indecomposables $B_w$. The precise definition of the $\Delta$-character function is %$\textup{ch}_\Delta : B \mapsto \sum_{y \in W} v^{\ell(y)} h_y(B) \delta_y$
$\textup{ch}_\Delta : B \mapsto \sum_{y \in W} v^{\ell(y)} h_y(B) \delta_y$, where $h_y(B) \in \mathbb{Z}_{\geq 0}[v, v^{-1}]$ is a kind of graded multiplicity coming from the $\Delta$-filtration for $B$ (see \cite[Theorem 5.10]{EMTW20} for details). For any indecomposable Soergel bimodule $B_w$, these multiplicities satisfy $h_y(B_w(k)) = v^{k-\ell(y)} h_{y, w}(v)$ by Soergel's Conjecture.\\
...
\noindent\textcolor{red}{Go through the $SL_2$ and $SL_3$ examples in detail to put everything together, it'd be good to see an example of the Kazhdan--Lusztig basis of a Iwahori--Hecke algebra and compare it to the indecomposables in the corresponding category of Soergel bimodules. I'd also like to write up some stuff about standard filtrations. I should probably start typing up Lie theory notes too.}\\
\newpage
...
\end{split}
\end{align*}
\noindent as well as two-sided ideals $\mathcal{I}_{Q_i}$ of $\mathcal{N}_{LV}^0(\textup{SL}(2, \mathbb{R}))$ generated by the identity morphisms in $Q_i$. Of course, one of the resulting quotients is trivial, so the subcategory $\mathcal{M}_{Q_1/Q_0} = \mathcal{M}_{Q_1}$ is already transitive. In fact, it is simple transitive, and equivalent to the cell module category corresponding to $B_s \in \obset(\mathbb{S}\textcat{Bim})$. For $\mathcal{M}_{Q_2/Q_1}$ and $\mathcal{M}_{Q_3/Q_2}$, we observe that $\mathcal{I}_{Q_i}$ consists of all morphisms in $\mathcal{N}_{LV}^0(\textup{SL}(2, \mathbb{R}))$ that factor through objects in $Q_i$ (with all other $\mathcal{I}_{Q_i}(X, Y)$ containing %, by definition, 
only the unique zero morphism). The idea here is that the quotient functor from $\mathcal{M}_{Q_2}$ to $\mathcal{M}_{Q_2/Q_1}$ sends $B_s$ to $0$, while the quotient functor from $\mathcal{M}_{Q_3}$ to $\mathcal{M}_{Q_3/Q_2}$ sends both $R_s$ and $B_s$ to $0$. As it happens, these categories are also already simple transitive, and the functor sending $R_s$ to $R$ defines an equivalence of module categories $\mathcal{M}_{Q_2/Q_1} \simeq \mathcal{M}_{Q_3/Q_2}$. These are equivalent to the cell module category for $R \in \obset(\mathbb{S}\textcat{Bim})$.\\% It is easy to be fooled by the fact that $R_s \otimes_R R_s \cong R \sim 0$, but one must remember that our Lusztig--Vogan module categories are {\em not} monoidal, and thus we cannot tensor by $R_s$ since it doesn't live in $\mathbb{S}\textcat{Bim}$!\\% \textcolor{red}{Write down what Victor and I discussed w.r.t. morphisms.}\\ %Let's now finally compute the weak composition quotients. %Well, for $\textup{L}_1$, observe that
%\begin{align*}
%\begin{split}
...
\begin{align*}
\begin{split}
R^s \otimes_{R^{s,t}} R(2) &\to R \otimes_R B_t \otimes_R B_s : 1 \otimes_{R^{s,t}} 1 \mapsto 1 \otimes_{R^t} 1 \otimes_{R^s} 1,\\
R &\to R \otimes_R B_t \otimes_R B_s : \hphantom{1 \otimes_{R^{s,t}} 1}\nhphantom{$1$} 1 \mapsto \frac{1}{2}((\alpha_t \otimes_{R^t} 1 + 1 \otimes_{R^t} \alpha_t) \otimes_{R^s} 1).
\end{split}
\end{align*}
\newpage
\noindent This was the case in the previous example too; the pattern is that we always have a ``longest indecomposable'' given in general by $R^{W_K} \otimes_{R^W} R$ and generated by $1 \otimes_{R^W} 1$. It can also be shown that $R \otimes_R B_t$ is generated by $1 \otimes_{R^t} 1$ and that $R_t \otimes_R B_s$ is generated by $1 \otimes_R 1 \otimes_{R^s} 1$. To see the former, observe that $\frac{1}{2}\alpha_s + \alpha_t \in R^s$ and $\alpha_s + \frac{1}{2}\alpha_t \in R^t$, whence
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent and similarly for $1 \otimes_R \alpha_t \otimes_{R^s} 1$. For the factor of $R_t$ on the left, we also have
\begin{align*}
\begin{split}
...
\end{tikzcd}.
\end{center}
\noindent Suppose we consider the filtration given by $Q_1 \coloneqq \{[L]\}$, $Q_2 \coloneqq \{[R_{ts}], [L]\}$, $Q_3 \coloneqq \{[R], [R_{ts}], [L]\}$ and $Q_4 \coloneqq \{[R_t], [R], [R_{ts}], [L]\}$. As before, these give us additive $\mathbb{S}\textcat{Bim}$-module subcategories $\mathcal{M}_i$ generated by the indecomposables in $Q_i$ and two-sided ideals $\mathcal{I}_{Q_i}$ generated by the identity morphisms in $Q_i$. Looking at the morphisms, we may verify that this results in four non-equivalent transitive module categories: $\mathcal{M}_{Q_1}$, $\mathcal{M}_{Q_2/Q_1}$, $\mathcal{M}_{Q_3/Q_2}$ and $\mathcal{M}_{Q_4/Q_3}$. It turns out that $\mathcal{M}_{Q_1}$ and $\mathcal{M}_{Q_4/Q_3}$ are already simple transitive, and are equivalent to the cell module categories corresponding to $B_{sts} \in \obset(\mathbb{S}\textcat{Bim})$ and $R \in \obset(\mathbb{S}\textcat{Bim})$, respectively. After applying \hyperref[SimpleQuotient]{Theorem \ref*{SimpleQuotient}} to $\mathcal{M}_{Q_2/Q_1}$ and $\mathcal{M}_{Q_3/Q_2}$, we end up with simple transitive module categories that are equivalent to the cell module categories corresponding to $B_t \in \obset(\mathbb{S}\textcat{Bim})$ and $B_s \in \obset(\mathbb{S}\textcat{Bim})$, respectively. \textcolor{red}{The morphisms that we kill when applying \hyperref[SimpleQuotient]{Theorem \ref*{SimpleQuotient}} are the ones underlined in green in the following image:}\\
\begin{center}
\includegraphics[width=0.85\linewidth]{PXL_20251215_012105744.jpg}
\end{center}
\end{example}
\newpage

\noindent\begin{lemma} Suppose the tori $T_K$ and $T$ are of equal rank. Then the corresponding Lusztig--Vogan category admits a ``longest indecomposable'' of the form $R^{W_K} \otimes_{R^W} R$.\\
\end{lemma}

\noindent\begin{proof} Clearly this is an indecomposable $(R^{W_K}, R)$-bimodule, as it is generated by $1 \otimes_{R^W} 1$. All that remains is to show that it is in our category. A well-known result from the theory of Soergel bimodules is that $B_{w_0} = R \otimes_{R^W} R(\ell(w_0))$, where $w_0$ is the longest element of $W$. Tensoring on the left by the standard $(R^{W_K}, R)$-bimodule $R \in \obset(\mathcal{N}_{LV}^0)$, we obtain $R \otimes_R B_{w_0} = R^{W_K} \otimes_{R^W} R(\ell(w_0))$. This completes the proof.
\end{proof}\\

\noindent\begin{lemma}\textup{(Anna's First Lemma).}\label{Anna1} For all $x \in W$ and $r \in S$, the short exact sequence
\begin{align*}
\begin{split}
R_x \otimes_R R_r(-1) \cong R_{xr}(-1) \hookrightarrow R_x \otimes_R B_r \twoheadrightarrow R_x(1)
\end{split}
\end{align*}
\noindent is a splitting of $(R^{W_K}, R)$-bimodules if and only if $x^{-1}(R^{W_K}) \subseteq R^r$.\\
\end{lemma}

\noindent\begin{lemma}\textup{(Anna's Second Lemma).}\label{Anna2} For all $y \in W$ and $r \in S$, we have an isomorphism $R_{yr} \otimes_R B_r \cong R_y \otimes_R B_r$ of graded $(R, R)$-bimodules.\\
\end{lemma}

\noindent\begin{lemma}\textup{(Anna's Third Lemma).}\label{Anna3} Let $M$ be a graded $(R^{W_K}, R)$-bimodule for which
\begin{enumerate}[label=\textup{(\roman*).}, ref=\textup{(\roman*)}, leftmargin=4\parindent]
\item $M$ is positively graded;
\item $M^0 \cong \mathbb{R}$;
\item $M$ is graded free of finite rank as a right $R$-module;
\item there exists some $m_0 \in M^0$ such that, for all $m \in M\setminus R^{W_K}\cdot m_0\cdot R$, there exists a non-zero $f \in R$ satisfying $m\cdot f \in R^{W_K}\cdot m_0\cdot R$.
\end{enumerate}
\noindent Then $M$ is indecomposable.\\
\end{lemma}

\noindent\begin{proposition}\textup{(Lusztig--Vogan Demazure Operator).} Consider some reflection $w \in W$ and let $y \in R$ be its corresponding root. We have an $(R^w, R^w)$-bimodule homomorphism $\partial_w : R \to R^w(-2)$,
\begin{align*}
\begin{split}
\partial_w : f \mapsto \frac{f - w(f)}{y},
\end{split}
\end{align*}
\noindent for all $f \in R$.\\
\end{proposition}

\noindent\begin{remark} A generalization of \hyperref[RSplitting]{Lemma \ref*{RSplitting}} tells us that for any $f \in R$, we have a splittng of $(R^w, R^w)$-bimodules $R \cong R^w \oplus R^wy \cong R^w \oplus R^w(-2)$ where $f + w(f) \in R^w$ and $f - w(f) \in R^wy$.\\
\end{remark}

\noindent\begin{proposition} Let $r \in S$. Then we have elements
\begin{align*}
\begin{split}
c_r &\coloneqq \frac{1}{2}(\alpha_r \otimes_{R^r} 1 + 1 \otimes_{R^r} \alpha_r),\\
d_r &\coloneqq \frac{1}{2}(\alpha_r \otimes_{R^r} 1 - 1 \otimes_{R^r} \alpha_r)
\end{split}
\end{align*}
\noindent of $B_r \in \obset(\mathbb{S}\textcat{Bim}(W, S))$ satisfying
\begin{align*}
\begin{split}
f\cdot c_r = c_r \cdot f\qquad\text{and}\qquad f\cdot d_r = d_r\cdot r(f),
\end{split}
\end{align*}
\noindent for all $f \in R$.\\
\end{proposition}

\noindent\textcolor{red}{I should write proofs for these. The proof for the last proposition is just a matter of using \hyperref[RSplitting]{Lemma \ref*{RSplitting}}.}
\newpage

\noindent\begin{example}\textup{($G_\mathbb{R} = \textup{SU}(3,1)$).} For this example, we won't spend as much time on the setup. Our real group will be $\textup{SU}(3,1)$. Continuing the pattern, we have $W = S_4 = \langle s, t, u \rangle$ and $W_K = S_3 = \langle s, t\rangle$. Recall (for my own sake) that
\begin{align*}
\begin{split}
...
\end{tikzpicture}\quad$\overset{-\otimes_R B_u}{\rightsquigarrow}$\quad\begin{tikzpicture}[baseline=-0.25cm, x=0.75cm,y=0.75cm]
\draw [line width=1pt, blue] (0,0) -- (2,-1);
\draw [line width=1pt, red] (0,0) -- (2,1);
\draw [line width=1pt, green] (2,1) -- (2,-1);
\draw (1.25,0) node [text=black] {$1$};
...
\draw [line width=1pt, red] (4,1) -- (4,-1);
\draw (4.75,0) node [text=black] {$0$};
\end{tikzpicture}\quad$\overset{-\otimes_R B_t}{\rightsquigarrow}$\quad\begin{tikzpicture}[baseline=-0.25cm, x=0.75cm,y=0.75cm]
\draw [line width=1pt, blue] (0,0) -- (2,-1);
\draw [line width=1pt, red] (0,0) -- (2,1);
\draw [line width=1pt, green] (2,1) -- (2,-1);
...
\begin{center}
$R \otimes_R B_u \otimes_R B_t =$\ \begin{tikzpicture}[baseline=-0.25cm, x=0.75cm,y=0.75cm]
\draw [line width=1pt, blue] (0,0) -- (2,-1);
\draw [line width=1pt, red] (0,0) -- (2,1);
\draw [line width=1pt, green] (2,1) -- (2,-1);
\draw (1.25,0) node [text=black] {$2$};
...
\draw [line width=1pt, red] (4,1) -- (4,-1);
\draw (4.75,0) node [text=black] {$0$};
\end{tikzpicture}\ \ \!$=$\ \begin{tikzpicture}[baseline=-0.25cm, x=0.75cm,y=0.75cm]
\draw [line width=1pt, blue] (0,0) -- (2,-1);
\draw [line width=1pt, red] (0,0) -- (2,1);
\draw [line width=1pt, green] (2,1) -- (2,-1);
\draw (1.25,0) node [text=black] {$1$};
...
\noindent\textcolor{red}{We can actually probably solve this using technology learned from the $\textup{Sp}(1, 1)$ example!}
\newpage
...
& & \textcolor{red}{?}\arrow[dl, shift right, "s"']\arrow[ur, "s"']\arrow[out=80, in=140, loop, looseness=5, "{t,u}"'] & & \textcolor{red}{?}\arrow[ul, "u"']\arrow[dr, shift right, "u"']\arrow[out=40, in=100, loop, looseness=5, "{s,t}"'] & &\\
...
\end{center}
\noindent\\[7.25\linespacing] In type $A_n$ for $n > 1$, we should always have four simple transitive module subcategories, each with multiplicity $1$. I don't know which cell module categories they all correspond to, but I think if we knew the cell structure in type $A_3$ we could figure out the pattern. It may be worth having a look at \cite[Chapter 22]{EMTW20}.
\end{example}
\newpage

\noindent\begin{example}\textup{($G_\mathbb{R} = G_{2(2)}$).} For more details on this setup, please refer to the appendix of my notes on the exceptional Lie group $G_2$. The Weyl group for $G_2$ is well known to be
\begin{align*}
\begin{split}
D_6 \coloneqq \langle s, t : s^2 = t^2 = (st)^6 = 1\rangle,
\end{split}
\end{align*}
\noindent the dihedral group of order $12$. In \cite[Theorem 1.10.1]{Yok25}, it is shown that the fixed-point subgroup of split $G_2$ under its Cartan involution is $(\textup{Sp}(1) \times \textup{Sp}(1))/\mathbb{Z}_2 \cong \textup{SO}(4)$. The fixed-point subgroup of complex $G_2$ under this Cartan involution is then $K = \textup{SL}(2, \mathbb{C}) \times \textup{SL}(2, \mathbb{C})$ -- the complexification of $\textup{SO}(4)$ -- giving us $W_K = \mathbb{Z}/2\mathbb{Z} \times \mathbb{Z}/2\mathbb{Z}$. Both $K$ and complex $G_2$ have a maximal torus of rank $2$, landing us in the equal rank case. Meanwhile, using the Atlas of Lie Groups and Representations software, we are able to determine that $W_K$ is generated by the roots $3\alpha_s+\alpha_t$ and $\alpha_s+\alpha_t$; that is, $W_K = \langle sts, tst\rangle = \{1, sts, tst, ststst\}$, whence it follows that $W_K\backslash W = \{W_K, W_Ks, W_Kt\}$. From here, we may determine the polynomial rings $R^{W_K}$, $R^s$ and $R^t$, which are nothing but
\begin{align*}
\begin{split}
R^{W_K} = R^{sts,tst} &= \mathbb{R}[(\alpha_s+\alpha_t)^2, (3\alpha_s+\alpha_t)^2],\\% = \mathbb{R}[\alpha_s(2\alpha_s+\alpha_t), \alpha_t(3\alpha_s+2\alpha_t)],\\
R^s &= \mathbb{R}[(3\alpha_s+2\alpha_t), \alpha_s^2],\\ 
R^t &= \mathbb{R}[(2\alpha_s+\hphantom{2}\alpha_t), \alpha_t^2].
\end{split}
\end{align*}
%We will prefer to work with the coordinate system
%\begin{align*}
%\begin{split}
%x \coloneqq \alpha_s + \alpha_t\quad\text{and}\quad y \coloneqq 3\alpha_s + \alpha_t,
%\end{split}
%\end{align*}
%\noindent which are the roots corresponding to $sts$ and $tst$, respectively. The polynomial rings $R^{W_K}$, $R^s$ and $R^t$ under this change of variables can be read off the root system below as
%\begin{align*}
%\begin{split}
%R^{W_K} = R^{sts,tst} &= \mathbb{R}[x^2, y^2],\\
%R^s &= \mathbb{R}[3x+y, (y-x)^2],\\ 
%R^t &= \mathbb{R}[x+y, (3x-y)^2].
%\end{split}
%\end{align*}
\noindent Quotienting the root system by $W_K$, we obtain
\begin{center}
\begin{tikzpicture}[scale=10, baseline=(current bounding box.center)]
\fill[fill=gray!20, trig format=rad] (0,0) -- ({cos(2*pi/6)/2},{sin(2*pi/6)/2}) -- ({0},{sin(2*pi/6)/2});
%
\draw[-latex] (0,0) -- (1/2,0);
\draw[-latex] (0,0) -- (-1/2,0);
\node at (1/2+0.05,0) {$\alpha_t$};
%
\draw[-latex, trig format=rad] (0,0) -- ({cos(5*pi/6)/sqrt(12)},{sin(5*pi/6)/sqrt(12)});
\draw[-latex, trig format=rad] (0,0) -- ({-cos(5*pi/6)/sqrt(12)},{-sin(5*pi/6)/sqrt(12)});
\node[trig format=rad] at ({cos(5*pi/6)*(1/sqrt(12)+0.05)},{sin(5*pi/6)*(1/sqrt(12)+0.05)}) {$\alpha_s$};
%
\draw[-latex, trig format=rad] (0,0) -- ({cos(pi/6)/sqrt(12)},{sin(pi/6)/sqrt(12)});
\draw[-latex, trig format=rad] (0,0) -- ({-cos(pi/6)/sqrt(12)},{-sin(pi/6)/sqrt(12)});
\node[trig format=rad] at ({cos(pi/6)*(1/sqrt(12)+0.075)},{sin(pi/6)*(1/sqrt(12)+0.075)}) {$\alpha_s + \alpha_t$};
%
\draw[-latex, trig format=rad] (0,0) -- ({0},{2*sin(5*pi/6)/sqrt(12)});
\draw[-latex, trig format=rad] (0,0) -- ({0},{-2*sin(5*pi/6)/sqrt(12)});
\node[trig format=rad] at ({0},{sin(5*pi/6)*(2/sqrt(12)+0.075)}) {$2\alpha_s + \alpha_t$};
%
\draw[-latex, trig format=rad] (0,0) -- ({cos(4*pi/6)/2},{sin(2*pi/6)/2});
\draw[-latex, trig format=rad] (0,0) -- ({-cos(4*pi/6)/2},{-sin(2*pi/6)/2});
\node[trig format=rad] at ({cos(4*pi/6)*(1/2+0.05)},{sin(2*pi/6)*(1/2+0.05)}) {$3\alpha_s + \alpha_t$};
%
\draw[-latex, trig format=rad] (0,0) -- ({cos(2*pi/6)/2},{sin(2*pi/6)/2});
\draw[-latex, trig format=rad] (0,0) -- ({-cos(2*pi/6)/2},{-sin(2*pi/6)/2});
\node[trig format=rad] at ({cos(2*pi/6)*(1/2+0.05)},{sin(2*pi/6)*(1/2+0.05)}) {$3\alpha_s + 2\alpha_t$};
%
\node[trig format=rad] at ({cos(5*pi/12)*(1/5)},{sin(5*pi/12)*(1/5)}) {\small $1$};
\node[trig format=rad] at ({cos(3*pi/12)*(1/5)},{sin(3*pi/12)*(1/5)}) {\small $s$};
\node[trig format=rad] at ({cos(7*pi/12)*(1/5)},{sin(7*pi/12)*(1/5)}) {\small $t$};
\node[trig format=rad] at ({cos(9*pi/12)*(1/5)},{sin(9*pi/12)*(1/5)}) {\small $ts$};
\node[trig format=rad] at ({cos(pi/12)*(1/5)},{sin(pi/12)*(1/5)}) {\small $st$};
\node[trig format=rad] at ({cos(11*pi/12)*(1/5)},{sin(11*pi/12)*(1/5)}) {\small $tst$};
\node[trig format=rad] at ({cos(-pi/12)*(1/5)},{sin(-pi/12)*(1/5)}) {\small $sts$};
\node[trig format=rad] at ({cos(13*pi/12)*(1/5)},{sin(13*pi/12)*(1/5)}) {\small $tsts$};
\node[trig format=rad] at ({cos(-3*pi/12)*(1/5)},{sin(-3*pi/12)*(1/5)}) {\small $stst$};
\node[trig format=rad] at ({cos(15*pi/12)*(1/5)},{sin(15*pi/12)*(1/5)}) {\small $tstst$};
\node[trig format=rad] at ({cos(-5*pi/12)*(1/5)},{sin(-5*pi/12)*(1/5)}) {\small $ststs$};
\node[trig format=rad] at ({cos(-7*pi/12)*(1/5)},{sin(-7*pi/12)*(1/5)}) {\small $ststst$};
%
\node[trig format=rad, white] at ({cos(2*pi/6)*(1/2+0.05)},{-sin(2*pi/6)*(1/2+0.05)}) {$3\alpha_1 + 2\alpha_2$};
\end{tikzpicture}
$\xrsquigarrow{\ \ W_K\backslash -\ \ }$
\begin{tikzpicture}[scale=5, baseline=(current bounding box.center)]
\draw[-latex, trig format=rad, white] (0,0) -- (0,{-sin(2*pi/6)/2});
\fill[fill=gray!20, trig format=rad] (0,0) -- ({cos(2*pi/6)/2},{sin(2*pi/6)/2}) -- ({0},{sin(2*pi/6)/2});
%
\draw[-latex, trig format=rad] (0,0) -- ({cos(pi/6)/sqrt(12)},{sin(pi/6)/sqrt(12)});
%
...
\end{tikzpicture},
\end{center}
\noindent where the long arrows corresponds to $s$-walls and the short arrows correspond to $t$-walls. Anna's method implies that $R \otimes_R B_s \overset{?}{\cong} R_s \otimes_R B_s$ and $R \otimes_R B_t \overset{?}{\cong} R_t \otimes_R B_t$ are indecomposable, while
\begin{align*}
\begin{split}
R_s \otimes_R B_t \overset{?}{\cong} R_s(-1) \oplus R_s(1)\quad\text{and}\quad R_t \otimes_R B_s \overset{?}{\cong} R_t(-1) \oplus R_t(1).
\end{split}
\end{align*}
\noindent\parbox{\textwidth}{If this is the case, then our lives will be considerably easier, as every indecomposable will be a direct summand of $R \otimes_R X$ for some Soergel bimodule $X$. Let's begin by showing that this is indeed so.\parfillskip=0pt}
\newpage
\noindent First, observe that $W_Kst = W_Ks$ and $W_Kts = W_Kt$, which imply that $R_{st} = R_s$ and $R_{ts} = R_t$ as $(R^{W_K}, R)$-bimodules. Thus \hyperref[Anna1]{Lemma \ref*{Anna1}} gives us the decompositions for $R_s \otimes_R B_t$ and $R_t \otimes_R B_s$, since
\begin{align*}
\begin{split}
s\cdot R^{W_K} = s\cdot\mathbb{R}[(\alpha_s+\alpha_t)^2, (3\alpha_s+\alpha_t)^2] = \mathbb{R}[(2\alpha_s+\hphantom{2}\alpha_t)^2, \alpha_t^2] = R^t,\\
t\cdot R^{W_K} = t\cdot\mathbb{R}[(\alpha_s+\alpha_t)^2, (3\alpha_s+\alpha_t)^2] = \mathbb{R}[(3\alpha_s+2\alpha_t)^2, \alpha_s^2] = R^s.
\end{split}
\end{align*}
\noindent By \hyperref[Anna2]{Lemma \ref*{Anna2}}, we know that $R \otimes_R B_s \cong R_s \otimes_R B_s$ and $R \otimes_R B_t \cong R_t \otimes_R B_t$ as $(R, R)$-bimodules, and so they are certainly isomorphic as $(R^{W_K}, R)$-bimodules. To show that they are indecomposable, we will use \hyperref[Anna3]{Lemma \ref*{Anna3}}. Let $M \coloneqq R \otimes_{R^s} R$. This is obviously positively graded and, and it's also graded free of finite rank since all Soergel bimodules are. We also have that $M^0 \cong \mathbb{R}\cdot (1\otimes_{R^s} 1) \cong \mathbb{R}$. To check the final condition, set $m_0 \coloneqq 1 \otimes_{R^s} 1$ and $M' \coloneqq R^{W_K}\cdot (1 \otimes_{R^s} 1) \cdot R$. We see that
\begin{align*}
\begin{split}
M' &= \mathbb{R}[(\alpha_s+\alpha_t)^2, (3\alpha_s+\alpha_t)^2] \otimes_{\mathbb{R}[3\alpha_s+2\alpha_t, \alpha_s^2]} \mathbb{R}[\alpha_s, \alpha_t]\\
&= \mathbb{R}[\alpha_s^2, \alpha_s\alpha_t, \alpha_t^2, (3\alpha_s+2\alpha_t)] \otimes_{R^s} R,
\end{split}
\end{align*}
\noindent since
\begin{align*}
\begin{split}
\alpha_s^2 &= \hphantom{\frac{9}{2}}(\alpha_s+\alpha_t)^2 + \frac{1}{3}(3\alpha_s+\alpha_t)^2 - \frac{1}{3}(3\alpha_s+2\alpha_t)^2,\\
\alpha_t^2 &= \frac{9}{2}(\alpha_s+\alpha_t)^2 + \frac{1}{2}(3\alpha_s+\alpha_t)^2 - \hphantom{\frac{1}{3}}(3\alpha_s+2\alpha_t)^2.
\end{split}
\end{align*}
\noindent Let's denote $R' \coloneqq \mathbb{R}[\alpha_s^2, \alpha_s\alpha_t, \alpha_t^2, (3\alpha_s+2\alpha_t)]$. Note that the even graded components of $R'$ and $R$ are equal; however, for odd $k \in \mathbb{N}$, we have only that $\dim((R')^k) = \dim(R^k)-1$. Consider any $p \in R\setminus R'$. Naturally $p$ splits as $p = p_\textup{even} + p_\textup{odd}$, where $p_\textup{even}$ consists only of elements from even graded pieces and $p_\textup{odd}$ consists only of elements from odd graded pieces. Acting on the right by $f \coloneqq 3\alpha_s + 2\alpha_t$, we have
\begin{align*}
\begin{split}
p\cdot (3\alpha_s + 2\alpha_t) = p_\textup{even}\cdot (3\alpha_s + 2\alpha_t) + p_\textup{odd}\cdot (3\alpha_s + 2\alpha_t).
\end{split}
\end{align*}
\noindent Of course $p_\textup{even}\cdot (3\alpha_s + 2\alpha_t) \in R'$, since both $p_\textup{even}$ and $3\alpha_s + 2\alpha_t$ are in $R'$. But we also have that $p_\textup{odd}\cdot (3\alpha_s + 2\alpha_t) \in R'$, since it will be even and $(R')^\textup{even} = R^\textup{even}$ by our earlier remark! In other words, we have shown that for any $m \coloneqq p \otimes_{R^s} q \in M\setminus M'$,
\begin{align*}
\begin{split}
m \cdot (3\alpha_s + 2\alpha_t) = p\cdot (3\alpha_s + 2\alpha_t) \otimes_{R^s} q \in M'
\end{split}
\end{align*}
\noindent as desired. The exact same proof shows that $R \otimes_R B_t$ is indecomposable, where we instead use the generator $2\alpha_s+\alpha_t$ of $R^t$ rather than $3\alpha_s+2\alpha_t$. This works because
\begin{align*}
\begin{split}
\alpha_s^2 &= \hphantom{\frac{15}{8}}\nhphantom{$\displaystyle\frac{1}{8}$}\frac{1}{8}(\alpha_s+\alpha_t)^2 + \frac{1}{8}(3\alpha_s+\alpha_t)^2 - \frac{2}{8}(2\alpha_s+\alpha_t)^2,\\
\alpha_t^2 &= \frac{15}{8}(\alpha_s+\alpha_t)^2 - \frac{1}{8}(3\alpha_s+\alpha_t)^2 - \frac{6}{8}(2\alpha_s+\alpha_t)^2,
\end{split}
\end{align*}
\begin{align*}
\begin{split}
\implies R^{W_K} \otimes_{R^t} R = \mathbb{R}[\alpha_s^2, \alpha_s\alpha_t, \alpha_t^2, (2\alpha_s + \alpha_t)] \otimes_{R^t} R.
\end{split}
\end{align*}
\noindent Let's keep going. Note that, using what we have shown, we have an $(R^{W_K}, R)$-bimodule isomorphism
\begin{align*}
\begin{split}
(R_s \otimes_R B_{tst}) \oplus R_s(-1) \oplus R_s(1) \cong R_s \otimes_R (B_{tst} \oplus B_t).
\end{split}
\end{align*}
\noindent In $\mathbb{S}\textcat{Bim}$, we have an $(R, R)$-bimodule isomorphism
\begin{align*}
\begin{split}
B_{tst} \oplus B_t \cong B_t \otimes_R B_s \otimes_R B_t.
\end{split}
\end{align*}
\newpage
\noindent Hence
\begin{align*}
\begin{split}
R_s \otimes_R (B_{tst} \oplus B_t) &\cong R_s \otimes_R B_t \otimes_R B_s \otimes_R B_t\\
&\cong (R_s(-1) \oplus R_s(1)) \otimes_R B_s \otimes_R B_t\\
&\cong (R_s \otimes_R B_s \otimes_R B_t)(-1) \oplus (R_s \otimes_R B_s \otimes_R B_t)(1).
\end{split}
\end{align*}
\noindent Our category is Krull--Schmidt, so every object has a unique decomposition into indecomposables up to reordering. Thus our isomorphism
\begin{align*}
\begin{split}
(R_s \otimes_R B_{tst}) \oplus R_s(-1) \oplus R_s(1) \cong (R_s \otimes_R B_s \otimes_R B_t)(-1) \oplus (R_s \otimes_R B_s \otimes_R B_t)(1)
\end{split}
\end{align*}
\noindent implies that $R_s \otimes_R B_s \otimes_R B_t$ contains $R_s$ as a direct summand. In particular, we must have some possibly decomposable $(R^{W_K}, R)$-bimodule $Q^{st}_\Delta$ satisfying
\begin{align*}
\begin{split}
R_s \otimes_R B_{tst} \cong Q^{st}_\Delta(-1) \oplus Q^{st}_\Delta(1)\qquad\text{and}\qquad R_s \otimes_R B_s \otimes_R B_t \cong Q^{st}_\Delta \oplus R_s.
\end{split}
\end{align*}
\noindent Well, $R_s$ lives inside $R \otimes_R B_s \otimes_R B_t$ as
\begin{align*}
\begin{split}
R_s \dhxrightarrow{\iota^{st}_0} R \otimes_R B_s \otimes_R B_t \xhookrightarrow{\pi^{st}_0} R_s,
\end{split}
\end{align*}
\noindent where
\begin{align*}
\begin{split}
\iota^{st}_0 : f \mapsto d_s \otimes_{R^t} s(f) \qquad\text{and}\qquad\pi^{st}_0 : f \otimes_{R^s} g \otimes_{R^t} h \mapsto s(\partial_t(s(f)g)h) = \partial_{sts}(fs(g))s(h).
\end{split}
\end{align*}
\noindent Note that these maps are well-defined $(R^{W_K}, R)$-bimodule homomorphisms, since
\begin{align*}
\begin{split}
\begin{array}{rrr}s((\alpha_s+\alpha_t)^2) &= &(2\alpha_s + \alpha_t)^2\\ s((3\alpha_s+\alpha_t)^2) &= &\alpha_t^2\end{array}\bigg\}\implies s(R^{W_K}) \subset R^t.
\end{split}
\end{align*}
\noindent We similarly have that $t(R^{W_K}) \subset R^s$, meaning this continues to apply for $R_t$ within $R \otimes_R B_t \otimes_R B_s$. From the $W$-graph and standard filtrations, we know that either $Q^{st}_\Delta$ is indecomposable or it is the direct sum of a new indecomposable with $R_s$.
\end{example}
\newpage

\noindent\begin{conjecture}\label{wgraphconjecture}\ 
\begin{enumerate}[label=\textup{(\roman*).}, ref=\textup{(\roman*)}, leftmargin=4\parindent]
\item\label{wgraphconjecturei} Let $w \in {}^KW$ be a right coset representative whose shortest expression $\underline{w}$ ends in $s \in S$. Then we have an isomorphism
\begin{align*}
\begin{split}
R_{\underline{w}s} \otimes_R B_s \cong R_{\underline{w}} \otimes_R B_s
\end{split}
\end{align*}
of indecomposable $(R^{W_K}, R)$-bimodules.
\item\label{wgraphconjectureii} All actions on the standard $(R^{W_K}, R)$-bimodule $R_w$ that are not covered by \ref{wgraphconjecturei} send $R_w$ to
\begin{align*}
\begin{split}
R_w(-1) \oplus R_w(1).
\end{split}
\end{align*}
\end{enumerate}
\noindent Define the {\em set of words} of an indecomposable $(R^{W_K}, R)$-bimodule $Q$ to be the set of all shortest $x \in W$ for which $Q$ is a direct summand of $R_w \otimes_R B_x$, for some $w \in W$. Define the {\em length} of $Q$ to be the length of the elements in its set of words.
\begin{enumerate}[label=\textup{(\roman*).}, ref=\textup{(\roman*)}, start=3, leftmargin=4\parindent]
\item\label{wgraphconjectureiii} Two indecomposable $(R^{W_K}, R)$-bimodules of length greater than $0$ are isomorphic if and only if their sets of words are the same.
\item\label{wgraphconjectureiv} Let $Q$ be an indecomposable $(R^{W_K}, R)$-bimodule of length $k$. Then either
\begin{align*}
\begin{split}
Q \otimes_R B_{s_i} \cong Q(-1) \oplus Q(1)
\end{split}
\end{align*}
or $Q \otimes_R B_{s_i}$ is a direct sum of indecomposables of length $k-1$ and $k+1$.\\
\end{enumerate}
\end{conjecture}

\noindent Parts \ref{wgraphconjecturei} and \ref{wgraphconjectureii} of \hyperref[wgraphconjecture]{Conjecture \ref*{wgraphconjecture}} have no choice but to be true due to how standard factors work in this setting. I think the same might be true for parts \ref{wgraphconjectureiii} and \ref{wgraphconjectureiv}.
\newpage

\noindent To-do: right now, a lot of these problems are kind of tough. I think the best way to proceed would be to think about the analogue of standard filtrations and formalize Anna's method / see what else we can learn from them.\\
