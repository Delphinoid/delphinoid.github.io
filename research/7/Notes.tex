...
\end{split}
\end{align*}
\noindent It can be shown that $B_t$ is generated by $1 \otimes_{R^s} 1$, that $R_t \otimes_R B_s$ is generated by $1 \otimes_R 1 \otimes_R 1$ and that $R^s \otimes_{R^W} R$ is generated by $1 \otimes_{R^{W}} 1$. To at least see why $B_s$ is decomposable as an $(R^s, R)$-bimodule, recall that we have an $(R^s, R^s)$-bimodule isomorphism $R \cong R^s \oplus R^s(-2)$ (and note that this is certainly {\em not} an isomorphism of $(R, R)$-bimodules, as $R^s$ is itself not an $(R, R)$-bimodule). By interpreting $B_s \coloneqq R \otimes_{R^s} R(1)$ as an $(R^s, R)$-bimodule, we can safely replace the $R$ on the left with the $(R^s, R^s)$-bimodule $R^s \oplus R^s(-2)$, giving us the $(R^s, R)$-bimodule isomorphism
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent\textcolor{red}{What are the filtrations? What module categories do we end up with?}\\

\noindent\textcolor{red}{What is ``$R^s \otimes_{R^W} R$'' in terms of our generators? How is $R_t \otimes_R B_s$ indecomposable when $B_s$ is decomposable?}
\end{example}
\newpage
