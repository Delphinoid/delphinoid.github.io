...
only the unique zero morphism). The idea here is that the quotient functor from $\mathcal{M}_{Q_2}$ to $\mathcal{M}_{Q_2/Q_1}$ sends $B_s$ to $0$, while the quotient functor from $\mathcal{M}_{Q_3}$ to $\mathcal{M}_{Q_3/Q_2}$ sends both $R_s$ and $B_s$ to $0$. As it happens, the functor sending $R_s$ to $R$ defines an equivalence of module categories $\mathcal{M}_{Q_2/Q_1} \simeq \mathcal{M}_{Q_3/Q_2}$; this must be the case, as $\mathbb{S}\textcat{Bim}$ only has two distinct cell birepresentations in type $A_1$. It is easy to be fooled by the fact that $R_s \otimes_R R_s \cong R \sim 0$, but one must remember that our Lusztig--Vogan module categories are {\em not} monoidal, and thus we cannot tensor by $R_s$ since it doesn't live in $\mathbb{S}\textcat{Bim}$!\\ %Let's now finally compute the weak composition quotients. %Well, for $\textup{L}_1$, observe that
%\begin{align*}
%\begin{split}
...
\end{split}
\end{align*}
\noindent implies that $R_s \otimes_R B_s \otimes_R B_t$ contains $R_s$ as a direct summand. In particular, we must have some possibly decomposable $(R^{W_K}, R)$-bimodule $Q_{st}$ satisfying
\begin{align*}
\begin{split}
R_s \otimes_R B_{tst} \cong Q_{st}(-1) \oplus Q_{st}(1)\qquad\text{and}\qquad R_s \otimes_R B_s \otimes_R B_t \cong Q_{st} \oplus R_s.
\end{split}
\end{align*}
\noindent Well, $R_s$ lives inside $R \otimes_R B_s \otimes_R B_t$ as
\begin{align*}
\begin{split}
R_s \dhxrightarrow{\iota^{st}_s} R \otimes_R B_s \otimes_R B_t \xhookrightarrow{\pi^{st}_s} R_s,
\end{split}
\end{align*}
\noindent where
\begin{align*}
\begin{split}
\iota^{st}_s : f \mapsto d_s \otimes_{R^t} f \qquad\text{and}\qquad\pi^{st}_s : f \otimes_{R^s} g \otimes_{R^t} h \mapsto s(\delta_t(s(f)g)h).
\end{split}
\end{align*}
...
\end{split}
\end{align*}
\noindent We similarly have that $t(R^{W_K}) \subset R^s$, meaning this continues to apply for $R_t$ within $R \otimes_R B_t \otimes_R B_s$. We claim that
\begin{align*}
\begin{split}
Q_{st} \cong \langle 1 \otimes_{R^s} 1 \otimes_{R^t} 1\rangle.
\end{split}
\end{align*}
\end{example}
\newpage
