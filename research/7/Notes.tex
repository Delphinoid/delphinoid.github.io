...
\begin{align*}
\begin{split}
\frac{1}{4}\left(-\frac{1}{2}\alpha_s - \alpha_t - \frac{3}{2}\alpha_u\right) \otimes_{R^{t,u}} \frac{3}{4}\left(\frac{3}{2}\alpha_s + \alpha_t + \frac{1}{2}\alpha_u\right) = \left(\alpha_s + \frac{1}{2}\alpha_t\right) \otimes_{R^{t,u}} 1,\\
\frac{3}{4}\left(\frac{1}{2}\alpha_s + \alpha_t + \frac{3}{2}\alpha_u\right) \otimes_{R^{t,u}} \frac{1}{4}\left(-\frac{3}{2}\alpha_s - \alpha_t - \frac{1}{2}\alpha_u\right) = \left(\alpha_u + \frac{1}{2}\alpha_t\right) \otimes_{R^{t,u}} 1,\\
\textcolor{red}{\alpha_t^2?}
\end{split}
\end{align*}
\newpage

\noindent The $W$-graph is hence\\[7\linespacing]
\begin{center}
\begin{tikzcd}[overlay, labels={outer sep=-1cm}, nodes={circle, minimum size=2.5cm, inner sep=-0.25cm, outer sep=-0.75cm}, cramped, column sep=-0.75cm, row sep=-0.75cm]
...
& & \textcolor{red}{R^t \otimes_{R^{t,u}} R?}\arrow[dl, shift right, "s"']\arrow[ur, "s"']\arrow[out=80, in=140, loop, looseness=5, "{t,u}"'] & & \textcolor{red}{?}\arrow[ul, "u"']\arrow[dr, shift right, "u"']\arrow[out=40, in=100, loop, looseness=5, "{s,t}"'] & &\\
...
\end{center}
\noindent\\[7.25\linespacing] The module categories generated by the leftmost and rightmost legs are equivalent, just like in the previous examples, leaving us with three distinct composition quotients. I would expect this pattern to repeat. This is also notably the first example where the Lusztig--Vogan category does not contain every simple transitive $\mathbb{S}\textcat{Bim}$-module category as a composition quotient, as type $A_3$ Coxeter systems have $5$ two-sided cells.
\end{example}
\newpage
...
\end{split}
\end{align*}
\noindent the dihedral group of order $12$. In \cite[Theorem 1.10.1]{Yok25}, it is shown that the fixed-point subgroup of complex $G_2$ under the Cartan involution $\gamma$ corresponding to split $G_2$ is $(\textup{Sp}(1) \times \textup{Sp}(1))/\mathbb{Z}_2 \cong \textup{SO}(4)$. This complexifies as $K = \textup{SL}(2, \mathbb{C}) \times \textup{SL}(2, \mathbb{C})$, giving us $W_K = \mathbb{Z}/2\mathbb{Z} \times \mathbb{Z}/2\mathbb{Z}$. Both $K$ and complex $G_2$ have a maximal torus of rank $2$, landing us in the equal rank case. At this stage, we are not sure how $W_K$ lives inside $W$. Indeed, there are three distinct copies of the Klein four-group\linebreak $\mathbb{Z}/2\mathbb{Z} \times \mathbb{Z}/2\mathbb{Z}$ living inside $D_6$; these are
\begin{align*}
\begin{split}
