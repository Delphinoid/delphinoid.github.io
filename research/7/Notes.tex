...
\noindent This is well-defined, as a generalization of \hyperref[RSplitting]{Lemma \ref*{RSplitting}} tells us that for any $f \in R$, we have an $(R^w, R^w)$-bimodule splittng $R \cong R^w \oplus R^wy \cong R^w \oplus R^w(-2)$ where $f + w(f) \in R^w$ and $f - w(f) \in R^wy$.\\
...
\begin{align*}
\begin{split}
f\cdot c_r = c_r \cdot r(f)\qquad\text{and}\qquad f\cdot d_r = d_r\cdot f,
\end{split}
\end{align*}
\noindent for all $f \in R$.
\end{proposition}
...
\begin{align*}
\begin{split}
R^{W_K} = R^{sts,tst} &= \mathbb{R}[(\alpha_s+\alpha_t)^2, (3\alpha_s+\alpha_t)^2],\\
...
\end{split}
\end{align*}
\noindent Well, $R_s$ lives inside $R_s \otimes_R B_s \otimes_R B_t$ as
\begin{align*}
\begin{split}
R_s \dhxrightarrow{c_s \otimes_R d_t} R_s \otimes_R B_s \otimes_R B_t \xhookrightarrow{\varphi_{st}} R_s,
\end{split}
\end{align*}
\noindent where
\begin{align*}
\begin{split}
\varphi_{st} : a \otimes_{R^s} b \otimes_R c \otimes_{R^t} d \mapsto 2\partial_{sts}(a)s(b\partial_t(c)d).
\end{split}
\end{align*}
\noindent\textcolor{red}{Copy working from written notes later.}
\end{example}
\newpage
