...
\noindent\begin{example} Consider the graded Hom-space $\morset^\bullet_{\mathbb{S}\textcat{Bim}}(B_s, B_s \otimes_R B_s)$. Since $\textnormal{ch}_\Delta(B_s) = b_s = \delta_s + v$ and $\textnormal{ch}_\Delta(B_s \otimes_R B_s) = b_s^2 = v^2 + v\delta_s + v^{-1}\delta_s + 1$, the standard form gives us
\begin{align*}
\begin{split}
(\textnormal{ch}_\Delta(B_s), \textnormal{ch}_\Delta(B_s \otimes_R B_s)) &= \tau(\omega(b_s)b_s^2) = \tau(b_s^3) = \tau((v + v^{-1})b_s^2) = \tau((v + v^{-1})^2b_s)\\
...
\noindent\begin{definition}\textnormal{(Demazure Operator).} For each $s \in S$, we define the corresponding {\em Demazure operator} to be the graded bimodule homomorphism $\partial_s : R \to R^s(-2)$ given by
\begin{align*}
\begin{split}
...
\noindent\begin{lemma}\textnormal{(\cite[Lemma 4.15]{EMTW20}).} Let $s \in S$. Then
\begin{enumerate}[label=\textnormal{(\roman*).}, leftmargin=4\parindent]
...
\end{split}
\end{align*}
\noindent\\[-1.2\linespacing] where $R_w$ is the $w$-standard $(R^{W_K}, R)$-bimdoule given by $R$ as a vector space with the left action given by left multiplication and the right action given by $s \cdot_w f \coloneqq s w(f)$, for all $s \in R_w$ and $f \in R$.\newpage
...
\end{center}
\noindent\\[-\baselineskip]
\noindent Recall that the indecomposable objects in $\mathbb{S}\textcat{Bim}(W, S)$ are, up to grading shifts and isomorphisms, given by Bott--Samelson bimodules of the form $B_w \coloneqq R \otimes_{R^w} R(1)$, for $w \in W$. Because module products distribute over direct sums, the indecomposables of $\mathcal{N}_{LV}^0(\textnormal{SL}(2, \mathbb{R}))$ are of the form\linebreak $R_{w_1} \otimes_R B_{w_2}$, for $w_1 \in {}^KW$ and $w_2 \in W$; that is, products of generators of $\mathcal{N}_{LV}^0(\textnormal{SL}(2, \mathbb{R}))$ with indecomposable Soergel bimodules. In our case with $\textnormal{SL}(2, \mathbb{R})$, we have four candidates:
\begin{alignat*}{2}
...
\end{split}
\end{align*}
\noindent\textcolor{red}{What are the filtrations?} The cell structure is
\begin{center}
\begin{tikzcd}[row sep=-4pt]
& \boxed{R_t}\arrow[dash]{dr} &\\
\boxed{R, B_t}\arrow[dash]{ur}\arrow[dash]{dr} & & \boxed{R_t \otimes B_s, R_{ts}}\\
& \boxed{R^s \otimes_{R^W} R}\arrow[dash]{ur} &
\end{tikzcd}.
\end{center}
\noindent Suppose we consider the filtration given by $Q_1 \coloneqq \{[R_{ts}]\}$, $Q_2 \coloneqq \{[R^s \otimes_{R^W} R], [R_{ts}]\}$,\linebreak $Q_3 \coloneqq \{[R_t], [R^s \otimes_{R^W} R], [R_{ts}]\}$ and $Q_4 \coloneqq \{[R], [R_t], [R^s \otimes_{R^W} R], [R_{ts}]\}$. \textcolor{red}{What module categories do we end up with?}\\
