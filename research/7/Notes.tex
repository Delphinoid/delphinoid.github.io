...
\noindent\begin{definition}\label{GeometricRepresentation}\textnormal{(Geometric Representation).} Let $(W, S)$ be a Coxeter system and $V$ the $\mathbbm{k}$-vector space with basis $\{\alpha_s : s \in S\}$, for $\mathbbm{k}$ a field of characteristic zero. Define a symmetric, bilinear form on $V$ by% = \textnormal{span}_\mathbb{R}(\{\alpha_s : s \in S\})$ by
\begin{align*}
\begin{split}
...
\noindent We remark that for any category $\mathcal{C}$ with $\mathbb{Z}$-graded objects, $\morset_\mathcal{C}(X(j), Y(i)) = \morset_\mathcal{C}(X, Y(i-j))$. We define the corresponding {\em graded Hom-space} by $\morset^\bullet_\mathcal{C}(X, Y) \coloneqq \bigoplus_{i\in\mathbb{Z}} \morset_\mathcal{C}(X, Y(i))$.\\
...
is free as a left graded $R$-module and as a right graded $R$-module, both with graded rank $\underline{\textnormal{rk}}(\morset_{\mathbb{S}\textcat{Bim}}(B, B')) = (\textnormal{ch}_\Delta(B), \textnormal{ch}_\Delta(B'))$, where $(-, -)$ denotes the standard form on $\mathscr{H}(W, S)$.\\
\end{theorem}
...
\noindent\textcolor{red}{Describe the diagrammatic Iwahori--Hecke category.}
