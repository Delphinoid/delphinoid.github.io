...
\end{align*}
\newpage
\noindent This was the case in the previous example too; in general, the pattern is that we always have a ``longest indecomposable'' given by $L \coloneqq R^{W_K} \otimes_{R^W} R$ and generated by $1 \otimes_{R^W} 1$. It can also be shown that $B_t$ is generated by $1 \otimes_{R^t} 1$ and that $R_t \otimes_R B_s$ is generated by $1 \otimes_R 1 \otimes_{R^s} 1$. To see the former, observe that $\alpha_t - \frac{1}{2}\alpha_s \in R^s$ and $\alpha_s - \frac{1}{2}\alpha_t \in R^t$, whence
\begin{align*}
\begin{split}
\frac{2}{3}\Big(\alpha_t - \frac{1}{2}\alpha_s\Big) \otimes_{R^t} 1 + 1 \otimes_{R^t} \frac{2}{3}\Big(2\alpha_s - \alpha_t\Big) = \alpha_s \otimes_{R^t} 1,\\
\frac{2}{3}\Big(2\alpha_t - \alpha_s\Big) \otimes_{R^t} 1 + 1 \otimes_{R^t} \frac{2}{3}\Big(\alpha_s - \frac{1}{2}\alpha_t\Big) = \alpha_t \otimes_{R^t} 1.
\end{split}
\end{align*}
\noindent As for the latter,
\begin{align*}
\begin{split}
&\ \frac{2}{5}\Big(\alpha_s - 2\alpha_t\Big) \otimes_R 1 \otimes_{R^s} 1 + 1 \otimes_R 1 \otimes_{R^s} \frac{2}{5}\Big(\frac{3}{2}\alpha_s - 3\alpha_t\Big)\\
=&\ 1 \otimes_R \frac{2}{5}\Big(\alpha_s + 3\alpha_t\Big) \otimes_{R^s} 1 + 1 \otimes_R \frac{2}{5}\Big(\frac{3}{2}\alpha_s - 3\alpha_t\Big) \otimes_{R^s} 1\\
=&\ 1 \otimes_R \alpha_s \otimes_{R^s} 1,
\end{split}
\end{align*}
\noindent and similarly for the remaining generator $\alpha_t$. To see why $B_s$ is decomposable as an $(R^s, R)$-bimodule, recall that there is an $(R^s, R^s)$-bimodule isomorphism $R \cong R^s \oplus R^s(-2)$ by \hyperref[RSplitting]{Lemma \ref*{RSplitting}}. Interpreting $B_s \coloneqq R \otimes_{R^s} R(1)$ as an $(R^s, R)$-bimodule and replacing the $R$ on the left with the $(R^s, R^s)$-bimodule $R^s \oplus R^s(-2)$, we have an $(R^s, R)$-bimodule isomorphism
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent Note that although $B_s$ is decomposable in $\mathcal{N}_{LV}^0(\textup{SU}(2, 1))$, it is still indecomposable in $\mathbb{S}\textcat{Bim}$, so $R_t \otimes_R B_s$ being indecomposable is not a contradiction! We can graph the action of $B_s$ and $B_t$ on each indecomposable, giving us\\[-4\linespacing]
\begin{center}
...
& & L\arrow[out=60, in=120, loop, looseness=5, "{s,t}"'] & &\\
...
\end{alignat*}
\noindent By the same reasoning as last time, $B_u$ is indecomposable, while the bimodules $B_s$ and $B_t$ decompose into copies of $R$. In addition, we again have a longest indecomposable given by $L = R^{s,t} \otimes_{R^{s,t,u}} R$.

The action of $B_s$, $B_t$ and $B_u$ on our generators is as follows.

Hence the $W$-graph tells us that\\[-4\linespacing]
\begin{center}
\begin{tikzcd}[labels={outer sep=-0.75cm}, nodes={circle, minimum size=2cm, inner sep=0cm, outer sep=-0.5cm}, cramped, column sep=-0.25cm, row sep=-0.25cm]
& & & L\arrow[out=60, in=120, loop, looseness=5, "{s,t,u}"'] & & &\\
& & Q_7 & & Q_8 & &\\
& B_u & & Q_5 & & Q_6 &\\
R\arrow[ur, shift right, "u"']\arrow[out=240, in=300, loop, looseness=5, "{s,t}"'] & & R_u\arrow[ul, shift right, "u"']\arrow[out=240, in=300, loop, looseness=5, "{s}"'] & & R_{ut} & & R_{uts}
% & & & Q_9 & & &\\
% & & Q_7 & & Q_8 & &\\
% & Q_4 & & Q_5 & & Q_6 &\\
% Q_2 & & Q_0 & & Q_1 & & Q_3
