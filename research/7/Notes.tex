...
\end{split}
\end{align*}
\noindent the dihedral group of order $12$. In \cite[Theorem 1.10.1]{Yok25}, it is shown that the fixed-point subgroup of split $G_2$ under its Cartan involution is $(\textup{Sp}(1) \times \textup{Sp}(1))/\mathbb{Z}_2 \cong \textup{SO}(4)$. The fixed-point subgroup of complex $G_2$ under this Cartan involution is then $K = \textup{SL}(2, \mathbb{C}) \times \textup{SL}(2, \mathbb{C})$ -- the complexification of $\textup{SO}(4)$ -- giving us $W_K = \mathbb{Z}/2\mathbb{Z} \times \mathbb{Z}/2\mathbb{Z}$ (for more details, see the appendix of my notes on $G_2$). Both $K$ and complex $G_2$ have a maximal torus of rank $2$, landing us in the equal rank case. At this stage, we are not sure how $W_K$ lives inside $W$. Indeed, there are three distinct copies of the Klein four-group $\mathbb{Z}/2\mathbb{Z} \times \mathbb{Z}/2\mathbb{Z}$ living inside $D_6$; these are
\begin{align*}
\begin{split}
