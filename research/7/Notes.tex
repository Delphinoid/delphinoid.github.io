...
\noindent Of course, we also know that $B_{s} \geq R, R_{s}$, since $B_{s}$ is isomorphic to both $[\textnormal{M}(B_{s})](R) = R \otimes_R B_{s}$ and $[\textnormal{M}(B_{s})](R_{s}) = R_{s} \otimes_R B_{s}$; thus we obtain the two complete filtrations
\begin{align*}
\begin{split}
