...
\item for each $1$-morphism $f \in \obset(\mathscr{C}(\textobj{i}, \textobj{j}))$, a $2$-morphism $\Phi_f : G(f) \circ_h \Phi_\textobj{i} \Rightarrow \Phi_\textobj{j} \circ_h \textup{F}(f)$ in $\morset(\mathscr{D}(\textup{F}(\textobj{i}), \textup{G}(\textobj{j})))$;
\end{enumerate}
\noindent such that
\begin{enumerate}[label=$\bullet$, leftmargin=4\parindent]
...
\noindent Let $\mathcal{L}_Z$ be the left cell of $Z \in \obset(\mathscr{C}(\textobj{s}, \textobj{t}))$ and define a multifinitary subbirepresentation $\textup{N}$ of the Yoneda birepresentation $\mathbb{P}_\textobj{s}$ given by restricting each $\mathbb{P}_\textobj{s}(\textobj{i}) = \mathscr{C}(\textobj{s}, \textobj{i})$ to the additive subcategory
\begin{align*}
\begin{split}
\textup{N}(\textobj{i}) \coloneqq \textcat{Add}(\{X \in \obset(\mathscr{C}(\textobj{s}, \textobj{i})) : X \geq_L \mathcal{L}_Z\}).
\end{split}
\end{align*}
\noindent We will refer to this as the \textit{pre-cell birepresentation of $\mathscr{C}$ associated to $\mathcal{L}_Z$}.\\
...
\noindent\begin{proof} The proof follows similarly to \hyperref[UniqueMaximalIdeal]{Proposition \ref*{UniqueMaximalIdeal}}, with $\textup{I}$ given by the sum of all $\mathscr{C}$-stable ideals of $\textup{N}$ that do not contain $\textup{id}_X$ for any $X \in \mathcal{L}$. Note that transitivity is no longer needed, as containing one such $\textup{id}_X$ means that you must contain $\textup{id}_Y$ for all $Y \in \mathcal{L}$ by $\mathscr{C}$-stability and the definition of left cells. Maximality also tells us that we must in fact include all identity morphisms corresponding to objects $X$ for which $X >_L \mathcal{L}$. This completes the proof.
\end{proof}\\
...
\noindent Unlike when going from transitive to simple, going from pre-cell to cell typically annihilates objects. In particular, we will annihilate every object $X$ for which $X >_L \mathcal{L}$.\newpage

\noindent Let $\textup{M}$ be a birepresentation of $\mathscr{C}$. For any $X \in \obset(\textup{M}(\textobj{i}))$, we define
\begin{align*}
\begin{split}
\textup{Ann}_\mathscr{C}(X) \coloneqq \{F \in \morset_\mathscr{C}^1(\textobj{i}, \textobj{j}) : \textobj{j} \in \obset(\mathscr{C}), [\textup{M}(F)](X) = 0\}
\end{split}
\end{align*}
...
\noindent The annihilator $\textup{Ann}_\mathscr{C}(\textup{M})$ gives us a two-sided biideal by taking all the $2$-morphisms between its $1$-morphisms.%$\
