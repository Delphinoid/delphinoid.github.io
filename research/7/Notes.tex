...
\end{split}
\end{align*}
\noindent Note that this is homeomorphic to $\mathbb{C}^\times \coloneqq \mathbb{C}\!\setminus\!\{0\}$, which we observe is the complexification of $\textnormal{SO}(2) \cong S^1$, the maximal compact subgroup (and, when considering  $\textnormal{SL}(2, \mathbb{R})$ as a Lie group, the maximal torus) of $\textnormal{SL}(2, \mathbb{R})$. Since $G^{\theta'}$ is connected, the identity component is just the entire group. However, $G^{\theta'}$ is a little awkward to work with; therefore, instead of using $G^{\theta'}$, consider the following.\newpage
...
\end{split}
\end{align*}
\noindent which is of course homeomorphic to $\mathbb{C}^\times$. Once again, since $G^\theta$ is connected, the identity component is just the entire group, and hence we will take $K \coloneqq G^\theta$. The benefit of looking at this realization of $\mathbb{C}^\times$ is that $G^\theta$ is also a maximal torus (when considering $\textnormal{SL}(2, \mathbb{C})$ as a complex algebraic group) given by the intersection of the pair of opposite Borel groups
\begin{align*}
\begin{split}
...
\noindent\textcolor{red}{When $T = G^\theta$, don't we always have that $W^\theta = \{e\}$? Using $W/W_K$ makes much more sense, so why is $W^\theta$ written in the most general setting?}\\
