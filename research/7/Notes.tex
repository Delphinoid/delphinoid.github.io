...
\noindent\begin{definition}\label{GeometricRepresentation}\textnormal{(Geometric Representation).} Let $(W, S)$ be a Coxeter system and $V$ the real vector space with basis $\{\alpha_s : s \in S\}$. Define a symmetric, bilinear form on $V$ by% = \textnormal{span}_\mathbb{R}(\{\alpha_s : s \in S\})$ by
\begin{align*}
\begin{split}
...
\noindent\begin{theorem}\textnormal{(Soergel Hom Formula).} For any two Soergel bimodules $B, B' \in \mathbb{S}\textcat{Bim}(W, S)$, %the graded vector space
...
\noindent This result, which was first proven by Elias and Williamson (\cite[Theorem 1.1]{EW14}), generalizes the Kazhdan--Lusztig Conjecture and finally gives us an algebraic categorification of the Hecke algebra.\\
...
\end{split}
\end{align*}
\noindent\\[-0.7\linespacing] where $P_{y,w}$ is a Kazhdan--Lusztig polynomial for the Kazhdan--Lusztig basis of $\mathscr{H}(W, S)$. The term $IH^{2i}_{X_y}(X_w)$ is the stalk of the $i$th hypercohomology of $\textnormal{IC}_w$ at a (equivalently, any) point of $X_y$, where $\textnormal{IC}_w$ denotes the intersection cohomology sheaves of $\overline{X_w}$ (\cite[13.13]{EMTW20}). This result was first shown in the appendix to \cite{KL79} (see Remark A11). One thing we may observe here is that we only sum over even graded pieces of the local intersection cohomology; the reason for this is that the odd graded pieces are all zero. \textcolor{red}{In fact, because we have a left action of $B$ on $G/B$, we can consider the equivariant cohomology $H_B^*(G/B)$, which we will find is a ring that is also graded in degree $2$. This is exactly where our ring $R$ comes from, and why we grade it the way that we do! (more detail?)}\newpage
\end{remark}

\noindent As it happens (by the \hyperref[FreydMitchell]{Freyd--Mitchell Embedding Theorem}, for instance), any category of Soergel bimodules has finite-dimensional vector spaces of morphisms and is therefore Krull--Schmidt by \hyperref[KrullSchmidt1]{Theorem \ref*{KrullSchmidt1}}. Unfortunately, however, by \hyperref[SBimIndecomposablesCardinality]{Corollary \ref*{SBimIndecomposablesCardinality}} it will only be {\em graded multifinitary}, as we only have finitely many indecomposable objects under both isomorphisms and grading shifts. Luckily for us, we can obtain an honestly multifinitary category by somehow moving the grading into the morphisms. Let's look at how this applies to $\mathbb{S}\textcat{Bim}(W, S)$.\\

\noindent We henceforth denote by $\mathbb{S}\textcat{Bim}_{(1)}(W, S)$ the category of Soergel bimodules that we have discussed up until now, and define a new category $\mathbb{S}\textcat{Bim}_\mathbb{Z}(W, S)$ as follows. We take the objects of $\mathbb{S}\textcat{Bim}_\mathbb{Z}(W, S)$ to be the ungraded objects of $\mathbb{S}\textcat{Bim}_{(1)}(W, S)$, and for any $B, B' \in \obset(\mathbb{S}\textcat{Bim}_\mathbb{Z}(W, S))$ we take the morphisms between them to be the graded Hom-space
\begin{align*}
\begin{split}
\morset_{\mathbb{S}\textcat{Bim}_\mathbb{Z}}(B, B') \coloneqq \morset^\bullet_{\mathbb{S}\textcat{Bim}_{(1)}}(B, B') = \bigoplus_{i\in\mathbb{Z}} \morset_{\mathbb{S}\textcat{Bim}_{(1)}}(B, B'(i)).
\end{split}
\end{align*}
\noindent Although there is certainly not an equivalence of categories between $\mathbb{S}\textcat{Bim}_{(1)}(W, S)$ and $\mathbb{S}\textcat{Bim}_\mathbb{Z}(W, S)$, they somehow encode precisely the same information. In particular, if we are given some $\mathbb{S}\textcat{Bim}_\mathbb{Z}(W, S)$, we can reconstruct $\mathbb{S}\textcat{Bim}_{(1)}(W, S)$ by taking its objects to be $\obset(\mathbb{S}\textcat{Bim}_\mathbb{Z}(W, S)) \times \mathbb{Z}$ and by defining
\begin{align*}
\begin{split}
\morset_{\mathbb{S}\textcat{Bim}_{(1)}}((B, j), (B', i)) \coloneqq \morset^{i-j}_{\mathbb{S}\textcat{Bim}_\mathbb{Z}}(B, B'),
\end{split}
\end{align*}
\noindent where $\morset^{i-j}_{\mathbb{S}\textcat{Bim}_\mathbb{Z}}(B, B')$ is the $i-j$th graded piece of $\morset_{\mathbb{S}\textcat{Bim}_\mathbb{Z}}(B, B')$. It's important to note, however, that this does not categorify the Hecke algebra (as we no longer have enough objects to have an isomorphism between $\textnormal{Gr}(\morset_{\mathbb{S}\textcat{Bim}_\mathbb{Z}}(B, B'))$ and $\mathscr{H}(W, S)$)! \textcolor{red}{Supposedly this is meant to categorify the Hecke algebra. I don't really understand. If we look at $B_s \otimes_R B_s$, this seems to be isomorphic to $B_s \oplus B_s$, which is kind of weird too. Somehow it seems like we only have the group algebra.}\\ %Moreover, note that $\mathbb{Z}[q, q^{-1}]$ is trivially a $\mathbb{Z}_+$-ring with $\mathbb{Z}_+$-basis $\{q^k : k \in \mathbb{Z}\}$. Define for each $k \in \mathbb{Z}$ an endofunctor $Q_k$ on $\mathbb{S}\textcat{Bim}_\mathbb{Z}(W, S)$ that takes objects $B$ to\\
...
Let $P \coloneqq \textnormal{Sym}(\textnormal{span}_\mathbb{R}(X(T_K)))$ be the symmetric algebra on the $\mathbb{R}$-span of the character lattice $X(T_K) \coloneqq \homset(T_K, \mathbb{C}^\times)$ of a maximal torus $T_K$ of $K$ contained in $B_K \coloneqq B \cap K$, which we grade in degree $2$%graded such that $\textnormal{span}_\mathbb{R}(X_*(T_K))$ has degree $2$
, and similarly let $R \coloneqq \textnormal{Sym}(\textnormal{span}_\mathbb{R}(X(T)))$ be the symmetric algebra on the $\mathbb{R}$-span of the character lattice of the unique maximal, $\theta$-stable torus $T \coloneqq Z_G(T_K)$ of $G$ containing $T_K$ (see \cite[Lemma 6.1.2]{LR22}), which we once again grade in degree $2$%graded such that $\textnormal{span}_\mathbb{R}(X_*(T))$ has degree $2$
