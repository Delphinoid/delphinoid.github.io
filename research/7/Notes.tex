...
\end{split}
\end{align*}
\noindent\\[-0.6\linespacing] We define $R^{W_K}$ to be the polynomials in $R$ that are invariant under action by $W_K$. By \cite[Remark 6.2.4]{LR22}, we also have in the equal rank case that $W = W^\theta$. In other words, we may write\\[-1.05\linespacing]
\begin{align*}
\begin{split}
\mathcal{N}_{LV}^0 = \langle R_w \otimes_R X : w \in {}^KW, X \in \obset(\mathbb{S}\textcat{Bim}(W, S))\rangle_{\oplus,\ominus,(1)},
\end{split}
\end{align*}
\noindent\\[-1.05\linespacing] where $R_w$ is the $w$-standard $(R^{W_K}, R)$-bimdoule given by $R$ as a vector space with left action given by left multiplication and right action given by $s \cdot_w r \coloneqq s w(r)$, for all $s \in R_w$ and $r \in R$.
\newpage
...
%\rule{0.5\linewidth}{1pt}
%\end{center}
\noindent Note again that $W^\theta = W$, as we would expect, while $R^{W_K} = R^s = \mathbb{R}[\alpha_s^2]$ is the graded subring of $R$ consisting only of polynomials in $\alpha_s$ of even degree. Now that we have a working definition for this Lusztig--Vogan module category, let's try to compute its Jordan--H\"{o}lder filtrations.\\[-1.5\baselineskip]
\begin{center}
\rule{0.5\linewidth}{1pt}
\end{center}
\noindent\\[-\baselineskip]
\noindent By \cite[Lemma 6.2.3 and Lemma 6.1.5]{LR22}, $\mathcal{N}_{LV}^0(\textnormal{SU}(2, 1))$ will be generated by the standard bimodules $R_w$, where $w$ is a coset representative for $W_K\backslash W = \{W_K, W_Kt, W_Kts\} \cong \mathbb{Z}/3\mathbb{Z}$. We therefore have nine candidates for the indecomposables of $\mathcal{N}_{LV}^0(\textnormal{SU}(2, 1))$: $R$, $R_t$, $R_{ts}$, $B_s$, $B_t$, $R_t \otimes_R B_s$, $R_t \otimes_R B_t$, $R_{ts} \otimes_R B_s$ and $R_{ts} \otimes_R B_t$. By our previous reasoning from the $\mathcal{N}_{LV}^0(\textnormal{SL}(2, \mathbb{R}))$ example, $R_t \otimes_R B_t \cong B_t$ and $R_{ts} \otimes_R B_s \cong R_t \otimes_R B_s$. %We claim that all of these objects are indecomposable except for $R_{ts} \otimes_R B_t$.
\textcolor{red}{Supposedly $B_s$ and $R_{ts} \otimes_R B_t$ are decomposable, with our last indecomposable being ``$R^s \otimes_{R^W} R$''. This doesn't make sense to me, and seems to contradict \cite[Lemma 6.2.3 and Lemma 6.1.5]{LR22} telling us that $R$, $R_t$ and $R_{ts}$ generate $\mathcal{N}_{LV}^0(\textnormal{SU}(2, 1))$. We expect only six indecomposables.}
\end{example}
\newpage
