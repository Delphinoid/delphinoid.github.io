...
\end{split}
\end{align*}
\noindent Let's denote $R' \coloneqq \mathbb{R}[\alpha_s^2, \alpha_s\alpha_t, \alpha_t^2, (3\alpha_s+2\alpha_t)]$. Note that the even graded components of $R'$ and $R$ are equal; however, for odd $k \in \mathbb{N}$, we have only that $\dim((R')^k) = \dim(R^k)-1$. Consider any $p \in R\setminus R'$. Naturally $p$ splits as $p = p_\textup{even} + p_\textup{odd}$, where $p_\textup{even}$ consists only of elements from even graded pieces and $p_\textup{odd}$ consists only of elements from odd graded pieces. Acting on the right by $3\alpha_s + 2\alpha_t$, we have
\begin{align*}
\begin{split}
