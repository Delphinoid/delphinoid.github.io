...
\begin{center}
\LARGE\scshape Classification of Fusion Categories\linebreak via Endomorphisms\noindent\\[-\linespacing]
...
\noindent\begin{definition}\textup{(Haagerup--Izumi Category).} Let $G$ be a finite Abelian group. Define a category with (isomorphism classes of) simple objects $[\alpha_g]$ and $[\alpha_g\rho]$, for all $g \in G$, and fusion rules given by
\begin{gather*}
...
\end{gather*}
\noindent We call this category the {\em Haagerup--Izumi category corresponding to $G$}.\\
\end{definition}
...
\end{split}
\end{align*}
\noindent Let's try to model this as a system of algebra endomorphisms. The equation above is essentially saying that we have two  In order to do this, we would need to satisfy the relation
\begin{align*}
\begin{split}
