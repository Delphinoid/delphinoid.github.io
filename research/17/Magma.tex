...
\end{split}
\end{align*}
\noindent Thus we are interested in the sets
\begin{align*}
\begin{split}
\text{Prim}^d(B) \coloneqq \{E \in \text{End}^d(B) : E^2 = E, \nexists E_1 \perp E_2 \ \text{s.t.\ \!}E = E_1 + E_2\}.
\end{split}
\end{align*}
\noindent These conditions give us a system of equations that we can solve using Magma.\\
...
\noindent One optimization we can make is to solve for primitive idempotents one degree at a time. However, solving the system of equations that define $\text{Prim}^d(B)$ gets computationally expensive quite fast, and even rank $4$ computations tend to require some tricks to perform. One significant optimization is to take $R$ over $\mathbb{Q}$ and realize its generators as large primes. This is a significant reduction that appears to make the computations tractable at the cost of a larger solution space. If we already know an idempotent, we can also use it to reduce the dimension of our ideal.
