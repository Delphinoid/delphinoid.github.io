...
\end{split}
\end{align*}
\noindent for $\chi_h : G \to \mathbbm{k}^\times$ and $U_\mathcal{K}(g) \in \textup{span}\{t_it_j'\}_{i,j = 1}^m = \mathcal{K}\mathcal{K}'$. We remark that $\chi_h(g)$ can never be zero, otherwise $U(g)s_h = 0 = s_h'U(g)$, which is nonsense since the invertibility of $U(g)$ would imply that $s_h = 0 = s_h'$. Now, note that $\mathcal{K}\mathcal{K}' = \textup{End}(\mathcal{K})$. Certainly $\mathcal{K}\mathcal{K}' \subseteq \textup{End}(\mathcal{K})$, since $t_it_j'\mathcal{K} \subseteq \mathcal{K}$; the converse direction comes by comparing dimensions. Thus $U_\mathcal{K} : G \to \textup{End}(\mathcal{K})$ defines a representation as the projection of $U(g)$ onto $\mathcal{K}\mathcal{K}'$.\\ %\textcolor{red}{We will later see that these $\chi_h$ are precisely the characters of $G$.}\\
...
\noindent\begin{lemma} We have $\rho(s_g), \rho(s_g') \in \mathcal{L}$ for all $g \in G$. In particular,
\begin{align*}
\begin{split}
...
\noindent\begin{corollary} \textcolor{red}{We have}
\begin{gather*}
\textcolor{red}{\chi_h(g) = \chi_g(h),}\\
\textcolor{red}{\alpha_h(U(g)) = \chi_h(g)^{-1} U(g)}
\end{gather*}
\noindent \textcolor{red}{for all $g, h \in G$.}\\
\end{corollary}

\noindent\begin{proof} \textcolor{red}{Recall that $\varphi V(g) = U_\mathcal{K}(g)\varphi$ and $\varphi^3(x) = \nu x$ by \hyperref[OneClickRotation]{Lemma \ref*{OneClickRotation}}. Thus}
\begin{align*}
\begin{split}
\textcolor{red}{U_\mathcal{K}(g)V(h) = \varphi V(g)\varphi^{-1}V(h) = \varphi V(g)\varphi^{-2}U_\mathcal{K}(h)\varphi}\\
\textcolor{red}{V(h)U_\mathcal{K}(g) = \varphi^{-1} U_\mathcal{K}(h)\varphi U_\mathcal{K}(g) = \varphi^{-1} U_\mathcal{K}(h)\varphi^2 V(g)\varphi^{-1}}
\end{split}
\end{align*}
\noindent \textcolor{red}{This completes the proof.}
\end{proof}\\
