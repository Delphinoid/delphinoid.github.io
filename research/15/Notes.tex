...
\noindent Let $\mathcal{C}$ be a pivotal near-group category with multiplicity $m$, whose invertible objects form the finite group $G$ of order $n$. Assume this category admits a realization as a subcategory of $\mathcal{E}(A)$ for some algebra $A$. That is, assume the existence of algebra endomorphisms $\rho$ and $\alpha_g$ of $A$ satisfying %the fusion rules
\begin{gather*}
[\alpha_g] \otimes [\alpha_h] = [\alpha_{gh}],\\
[\alpha_g] \otimes [\rho] = [\rho] \otimes [\alpha_g] = [\rho],\\
[\rho] \otimes [\rho] = \bigoplus_{g \in G} [\alpha_g] \oplus [\rho]^{\oplus m}.
...
\noindent\begin{lemma}\label{OneClickRotation} The map $\varphi : \mathcal{K} \to \mathcal{K}$ given by
\begin{align*}
\begin{split}
...
\begin{align*}
\begin{split}
\varphi^3(x) = \nu x,\qquad\varphi(\alpha_g(\varphi^{-1}(x))) = \varphi(V(g)\varphi^{-1}(x)) = U_\mathcal{K}(g)x
\end{split}
\end{align*}
\noindent and
\begin{align*}
\begin{split}
\varphi(U_\mathcal{K}(g)\varphi^{-1}(x)) = \varphi^{-1}(V(g)\varphi(x)) = \varphi^{-1}(\alpha_g(\varphi(x))) = \rho(U(g)) x U(g)^{-1}
\end{split}
\end{align*}
...
\begin{align*}
\begin{split}
{}^*y = \frac{d^\frac{3}{2}}{\nu}\rho^2(s_e')\rho^2(x)\rho(s_e)s_e \in \mathcal{K}.
\end{split}
\end{align*}
...
\begin{align*}
\begin{split}
\varphi'^3(y) = \nu y,\qquad\varphi'(\alpha_g(\varphi'^{-1}(y))) = \varphi'(\varphi'^{-1}(y)V(g)^{-1}) = yU_\mathcal{K}(g)^{-1}
\end{split}
\end{align*}
\noindent and
\begin{align*}
\begin{split}
\varphi'(\varphi'^{-1}(y)U_\mathcal{K}(g)^{-1}) = \varphi'^{-1}(\varphi'(y)V(g)^{-1}) = \varphi'^{-1}(\alpha_g(\varphi'(y))) = U(g) y\rho(U(g)^{-1})
\end{split}
\end{align*}
...
\noindent We have thus shown that $\rho$ and $\alpha_g$ are endomorphisms of the Leavitt algebra, as they are given by noncommutative polynomials in our Leavitt algebra generators. In doing so, we have chosen
\begin{enumerate}[label=\textup{(\roman*).}, ref=\textup{(\roman*)}, leftmargin=4\parindent]
...
\end{enumerate}
\noindent We only assume that $\mathcal{C}$ is pivotal so that $\nu\varphi$ is order $3$ and hence $\varphi \circ U_\mathcal{K}(g) \circ \varphi^{-1} = \varphi^{-1} \circ V(g) \circ \varphi$. \textcolor{red}{How do these choices affect things? I should add diagrams.}\\

\noindent Recall that the Leavitt algebra $\mathcal{L}$ admits an involutive antiautomorphism $-' : \mathcal{L} \to \mathcal{L}$ that exchanges each $s_g$ and $t_i$ with their primed versions and vice versa. Note that $\rho$, $\alpha_g$ and $\varphi$ are compatible with this map; in particular, $\rho(-)' = \rho(-')$, $\alpha_g(-)' = \alpha_g(-')$ and $\varphi(-)' = \varphi'(-')$. We shall henceforth define $\varphi$ and $l$ on $\mathcal{K}'$ via $\varphi'$ and $l'$, respectively. \textcolor{red}{Double-check this stuff. Maybe replace mentions of $\varphi'$ and $l'$ in what follows.}\\
...
\noindent\begin{theorem} If $d$ is irrational, then
\begin{enumerate}[label=\textup{(\roman*).}, ref=\textup{(\roman*)}, leftmargin=4\parindent]
...
\begin{align*}
\begin{split}
\rho(U(g))(S + T) = \sum_{h \in G}\rho(U(g))s_hs_h' + T\rho(U(g))T.
\end{split}
\end{align*}
...
\begin{align*}
\begin{split}
U'(g) &= \sum_{i,j=1}^m U'(g)_{ij}t_it_j' = \sum_{i,j=1}^m\sum_{p,q=1}^m{U_\mathcal{K}(g)_{pq}s_e't_i'\rho(t_p)(t_q')t_js_et_it_j'}\\
&= 
\end{split}
\end{align*}
...
%In other words, there exists a representation $U' : G \to \textup{End}(\mathcal{K})$.
\end{proof}\\
