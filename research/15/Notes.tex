...
\noindent Let $\mathcal{C}$ be a fusion category over $\mathbbm{k}$ with representatives $\{X_i\}_{i \in \Gamma}$ of isomorphism classes of simple objects, and choose bases for each multiplicity space $H_{i,j}^l := \homset_\mathcal{C}(X_l, X_i \otimes X_j)$. The {\em (quantum) $6j$-symbols of $\mathcal{C}$} are the matrix blocks
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent of the change-of-basis matrices $\Phi_{i_1,i_2,i_3} \coloneqq \bigoplus_{i_4 \in \Gamma} \Phi_{i_1,i_2,i_3}^{i_4}$, and are given by
\begin{align*}
\begin{split}
...
\begin{align*}
\begin{split}
u(U,V,W) : \bigoplus_{j \in \Gamma} (\homset_\mathcal{C}(U, X_j \otimes W) \otimes_\mathbbm{k} \homset_\mathcal{C}(X_j, V)) \to \homset(U, V \otimes W),\\
v(U,V,W) : \bigoplus_{l \in \Gamma} (\homset_\mathcal{C}(U, V \otimes X_{l\nhphantom{$_{l}$}\hphantom{j}\ \!\!}) \otimes_\mathbbm{k} \homset_\mathcal{C}(X_{l\nhphantom{$_{l}$}\hphantom{j}\ \!\!}, W)) \to \homset(U, V \otimes W)
\end{split}
\end{align*}
\noindent are the maps defined by
\begin{align*}
\begin{split}
u(U, V, W) \coloneqq \bigoplus_{j \in \Gamma}u_j(U, V, W)\quad\text{and}\quad v(U, V, W) \coloneqq \bigoplus_{l \in \Gamma}v_l(U, V, W),
\end{split}
\end{align*}
\noindent\\[-0.7\linespacing] with
\begin{align*}
\begin{split}
[u_j(U, V, W)](f \otimes_\mathbbm{k} g) \coloneqq (g \otimes \id_W) \circ f\quad\text{and}\quad [v_l(U, V, W)](f' \otimes_\mathbbm{k} g') \coloneqq (\id_V \otimes g') \circ f'
\end{split}
\end{align*}
\noindent for all $f \in \homset_\mathcal{C}(U, X_j \otimes W)$, $g \in \homset_\mathcal{C}(X_j, V)$, $f' \in \homset_\mathcal{C}(U, V \otimes X_l)$ and $g \in \homset_\mathcal{C}(X_l, W))$. Recall now the Yoneda embedding
\begin{align*}
\begin{split}
...
\begin{align*}
\begin{split}
X \otimes X \cong \mathbbm{1}.
\end{split}
\end{align*}
\noindent Let's start by computing the change-of-basis matrix $\Phi_{X,X,X}$. This matrix can be written in block diagonal form as
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent but $(X \otimes X) \otimes X \cong X \cong X \otimes (X \otimes X)$, so $\Phi_{X,X,X}^\mathbbm{1}$ is just the isomorphism of $0$-dimensional vector spaces; that is, $\Phi_{X,X,X}^\mathbbm{1} = 0$. Let's now compute $\Phi_{X,X,X}^X$. %Denote $X \otimes X$ by $\widetilde{\mathbbm{1}}$.
...
\begin{align*}
\begin{split}
\iota_{X \otimes (X \otimes X)}^X \coloneqq (\id_X \otimes \iota_{X,X}^{{\mathbbm{1}}}) \circ \rho_X^{-1} = [v_{{\mathbbm{1}}}(X, X, X \otimes X)](\rho_X^{-1} \otimes_\mathbbm{k} \iota_{X,X}^{{\mathbbm{1}}})
\end{split}
\end{align*}
\noindent for $\homset(X, (X \otimes X) \otimes X)$ and $\homset(X, X \otimes (X \otimes X))$, respectively. These are once again both $1$-dimensional spaces, so let's define a constant $\mu_1 \in \mathbbm{k}$ for which
\begin{align*}
\begin{split}
\Phi_{X,X,X}^X(\iota_{(X \otimes X) \otimes X}^X) \eqqcolon \mu_1 \cdot \iota_{X \otimes (X \otimes X)}^X.
\end{split}
\end{align*}
\noindent In order to determine $\mu_1$, we need to use the pentagon equation. To this end, we first choose bases for the Hom-spaces
\begin{align*}
\begin{split}
...
\begin{align*}
\begin{split}
\Phi_{{\mathbbm{1}},X,X}^{\mathbbm{1}}(\iota_{({\mathbbm{1}} \otimes X) \otimes X}^{\mathbbm{1}}) &\eqqcolon \mu_2 \cdot \iota_{{\mathbbm{1}} \otimes (X \otimes X)}^{\mathbbm{1}},\\
\Phi_{X,{\mathbbm{1}},X}^{\mathbbm{1}}(\iota_{(X \otimes {\mathbbm{1}}) \otimes X}^{\mathbbm{1}}) &\eqqcolon \mu_3 \cdot \iota_{X \otimes ({\mathbbm{1}} \otimes X)}^{\mathbbm{1}},\\
\Phi_{X,X,{\mathbbm{1}}}^{\mathbbm{1}}(\iota_{(X \otimes X) \otimes {\mathbbm{1}}}^{\mathbbm{1}}) &\eqqcolon \mu_4 \cdot \iota_{X \otimes (X \otimes {\mathbbm{1}})}^{\mathbbm{1}}.
\end{split}
\end{align*}
...
\mu_1 \cdot \iota_{(X \otimes (X \otimes X)) \otimes X}^{\mathbbm{1}}\arrow[d, "\alpha_{X,X \otimes X,X}"'] & & \mu_2 \cdot \iota_{(X \otimes X) \otimes (X \otimes X)}^{\mathbbm{1}}\arrow[d, "\alpha_{X \otimes X,X,X}"]\\
\mu_1\mu_3 \cdot \iota_{X \otimes ((X \otimes X) \otimes X)}^{\mathbbm{1}}\arrow[r, "\alpha_{X,X,X}\otimes\id_X"'] & \mu_1^2\mu_3 \cdot \iota_{X \otimes (X \otimes (X \otimes X))}^{\mathbbm{1}}\arrow[r, equal] & \mu_2\mu_4 \cdot \iota_{X \otimes (X \otimes (X \otimes X))}^{\mathbbm{1}}\\
\end{tikzcd}.
\end{center}
\newpage
\noindent .
