...
\end{split}
\end{align*}
\noindent \textcolor{red}{which is cohomologous to the trivial $3$-cocycle (proof? check section 3.1 of\\ https://8426thmy.github.io/research/fusion\_category\_modules\_phd/phd\_notes.pdf)}.\\ This completes the proof.
\end{proof}\\
...
\end{split}
\end{align*}
\noindent for all $g, h \in G$ and $i, j \in \{1, \dots, m\}$. There are in general no canonical choices for these elements, so we will proceed as follows. Because both $s_e \in \homset_{\mathcal{E}(A)}(\id, {\rho \otimes \rho})$ and $s_e' \in \homset_{\mathcal{E}(A)}({\rho \otimes \rho}, \id)$ live in $1$-dimensional spaces, we can choose them to be scalar multiples of the coevaluator and evaluator, normalized with respect to the direct sum condition $s_e's_e = 1$. That is,
\begin{align*}
\begin{split}
s_e \coloneqq \frac{1}{\csqrt{d}}\coev_\rho\qquad\text{and}\qquad s_e' \coloneqq \frac{1}{\csqrt{d}}(\ev_{\rho^*} \circ (\psi_\rho \otimes \textup{id}_\rho)) = \frac{\nu}{\csqrt{d}}\ev_\rho,
\end{split}
\end{align*}
\noindent where $\psi : \rho \to \rho^{**}$ is the pivotal structure corresponding to multiplication by $\nu \in \{-1, 1\}$, the Frobenius--Schur indicator of $\rho$. This will force
\begin{align*}
\begin{split}
\rho(s_e')s_e = \frac{\nu}{d}(\textup{id}_\rho \otimes \ev_\rho) \circ (\coev_\rho \otimes \textup{id}_\rho) = \frac{\nu}{d} = \frac{\nu}{d}(\ev_\rho \otimes \textup{id}_\rho) \circ (\textup{id}_\rho \otimes \coev_\rho) = s_e'\rho(s_e).
\end{split}
\end{align*}
...
\noindent We will find it convenient now to denote both $\mathcal{K} \coloneqq \homset_{\mathcal{E}(A)}(\rho, {\rho \otimes \rho}) = \text{span}_\mathbbm{k}\{t_1, \dots, t_m\}$ and $\mathcal{K}' \coloneqq \homset_{\mathcal{E}(A)}({\rho \otimes \rho}, \rho) = \text{span}_\mathbbm{k}\{t_1', \dots, t_m'\}$, and let
\begin{align*}
\begin{split}
\mathcal{K}^p\mathcal{K}'^q \coloneqq \text{span}_\mathbbm{k}\{t_{i_1}t_{i_2} \cdots t_{i_p}t_{j_1}'t_{j_2}' \cdots t_{j_q}'\}_{i_1,\dots,i_p,j_1,\dots,j_q=1}^m
\end{split}
\end{align*}
...
\end{split}
\end{align*}
\noindent for $\chi_h : G \to \mathbbm{k}^\times$ and $U_\mathcal{K}(g) \in \text{span}\{t_it_j'\}_{i,j = 1}^m = \mathcal{K}\mathcal{K}'$. We remark that $\chi_h(g)$ can never be zero, otherwise $U(g)s_h = 0 = s_h'U(g)$, which is nonsense since the invertibility of $U(g)$ would imply that $s_h = 0 = s_h'$. Now, note that $\mathcal{K}\mathcal{K}' = \text{End}(\mathcal{K})$. Certainly $\mathcal{K}\mathcal{K}' \subseteq \text{End}(\mathcal{K})$, since $t_it_j'\mathcal{K} \subseteq \mathcal{K}$; the converse direction comes by comparing dimensions. Thus $U_\mathcal{K} : G \to \text{End}(\mathcal{K})$ defines a representation as the projection of $U(g)$ onto $\mathcal{K}\mathcal{K}'$.\\ %\textcolor{red}{We will later see that these $\chi_h$ are precisely the characters of $G$.}\\

\noindent Another consequence of the definition of $\mathcal{K}$ is that $\alpha_g(x) \in \mathcal{K}$ for all $x \in \mathcal{K}$, and similarly $\alpha_g(y) \in \mathcal{K}'$ for all $y \in \mathcal{K}'$. We must therefore have an element $V(g) \in \mathcal{K}\mathcal{K}'$ satisfying $\alpha_g(x) = V(g)x$ for all $x \in \mathcal{K}$, defining a final representation $V : G \to \text{End}(\mathcal{K})$. In fact, $1 = \alpha_g(t_i't_i) = \alpha_g(t_i')V(g)t_i$ tells us that $\alpha_g(y) = yV(g)^{-1}$ for all $y \in \mathcal{K}'$, as both $V(g)$ and $t_i$ have unique left inverses.\\% In particular, $\alpha_g(U_\mathcal{K}(h)) = V(g)U_\mathcal{K}(h)V'(g)$.\\ %In particular, we have the following result.\\
...
%\noindent \textcolor{red}{Suppose we now let $V(g) : x \mapsto \id_{\alpha_g} \otimes x$ for all $x \in \homset_{\mathcal{E}(A)}(\rho, {\rho \otimes \rho})$, which defines another representation $V : G \to \text{End}(\homset_{\mathcal{E}(A)}(\rho, {\rho \otimes \rho}))$.}\\
...
\end{split}
\end{align*}
\noindent Because the pivotal structure $\psi_\rho : \rho \to {}^{**}\rho$ is multiplication by $\nu$, we have that ${}^{**}x = \nu^2x\nu^{-1}$, whence $\varphi^3(x) = \nu x$. Now, using the normalization relation $U(g)^{-1}s_e = s_e$ and the definition of morphisms,
\begin{align*}
\begin{split}
