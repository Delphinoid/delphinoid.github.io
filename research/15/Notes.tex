...
\end{split}
\end{align*}
\noindent for $p, q \in \mathbb{N}$. In other words, $\mathcal{K}^p\mathcal{K}'^q \cong \mathcal{K}^{\otimes p} \otimes (\mathcal{K}')^{\otimes q}$.\\
...
\end{split}
\end{align*}
\noindent for $\chi_h : G \to \mathbb{T}$ and $U_\mathcal{K}(g) \in \text{span}\{t_it_j'\}_{i,j = 1}^m$. Moreover, because $s_h'x = 0$ for all $x \in \mathcal{K}$, we have that $U(g)x = U_\mathcal{K}(g)x \in \mathcal{K}$ for all $x \in \mathcal{K}$. In other words, $U_\mathcal{K} : G \to \text{End}(\mathcal{K})$ itself defines a representation by restriction, hence the notation. %\textcolor{red}{We will later see that these $\chi_h$ are precisely the characters of $G$.}\\
Finally, for each $g \in G$, we define a matrix of the form $V(g) : x \mapsto \alpha_g(x)$, which defines yet another representation $V : G \to \text{End}(\mathcal{K})$.\\
...
\begin{align*}
\begin{split}
\varphi(\alpha_g(\varphi^{-1}(x))) = U_\mathcal{K}(g)x\qquad\text{and}\qquad \varphi^{-1}(\alpha_g(\varphi(x))) = \rho(U(g)) x U(g)^{-1}
\end{split}
\end{align*}
...
&= d^2\rho^2(s_e')\rho(s_e')s_exs_e = d\nu\rho^2(s_e')xs_e = d\nu x\rho(s_e')s_e = \nu^2x = x.
\end{split}
\end{align*}
...
&= d^2\rho(s_e')xs_e'\rho(s_e)\rho(s_e) = d\nu\rho(s_e')x\rho(s_e) = d\nu\rho(s_e')\rho^2(s_e)x = \nu^2x = x.
\end{split}
\end{align*}
...
\begin{align*}
\begin{split}
{}^{**}x = \frac{d^3}{\nu^3}\rho^2(s_e')\rho^3(s_e')\rho^4(s_e')\rho^3(x)\rho^2(s_e)\rho(s_e)s_e.
\end{split}
\end{align*}
\noindent Because the pivotal structure $\psi_\rho : \rho \to {}^{**}\rho$ is multiplication by $\nu$, we have that $x = \nu^{-2}{}^{**}x\nu$. But $\nu = \pm 1$, so this is nothing but $\varphi^3(x)$! We now prove the remaining equalities. Using the normalization relation $U(g)^{-1}s_e = s_e$, we observe that
\begin{align*}
\begin{split}
...
&= d^2U(g)\rho^2(s_e')\rho(s_e')\rho^2(x)\rho^2(s_e)s_e = U(g)x = U_\mathcal{K}(g)x. %\frac{d^2}{\nu^2}\rho^2(s_e')\rho(\alpha_g(s_e'))\rho^2(x)\rho^2(s_e)s_e\\
...
\end{split}
\end{align*}
\noindent Finally, using the normalization relation $U(g)s_e = s_e$, we have
\begin{align*}
\begin{split}
...
% &= 
\varphi^{-1}(\alpha_g(\varphi(x))) &= d^2s_e'\rho(\alpha_g(\rho^2(s_e')\rho(x) s_e))\rho(s_e) = d^2s_e'U(g)\rho^3(s_e')\rho^2(x)\rho(s_e)U(g)^{-1}\rho(s_e)\\
&= 
...
\noindent We want to show that $\rho$ and $\alpha_g$ restrict to endomorphisms of the Leavitt algebra $\mathcal{L} \coloneqq \mathcal{L}_{m+n}$, with generators $s_g, s_g', t_i, t_i'$ for $g \in G$ and $i \in \{1, \dots, m\}$.\\
...
\end{split}
\end{align*}
\noindent Similarly, $\rho(s_g') = U(g)\rho(s_e')U(g)^{-1}$. Thus it suffices to show how $\rho$ acts on $s_e$ and $s_e'$. To this end, we begin by recalling that
\begin{align*}
\begin{split}
1 = \sum_{h \in G}{s_hs_h'} + \sum_{i=1}^m{t_it_i'}.
\end{split}
\end{align*}
\noindent Multiplying both sides by $\rho(s_e)$ on the right and using $s_e'\rho(s_e) = \nu/d$ from before, we obtain
\begin{align*}
\begin{split}
\rho(s_e) &= \sum_{h \in G}{s_hs_h'\rho(s_e)} + \sum_{i=1}^m{t_it_i'\rho(s_e)}\\
&= \sum_{h \in G}{\alpha_h(s_es_e'\rho(s_e))} + \sum_{i=1}^m{t_it_i'\rho(s_e)}\\
&= \sum_{h \in G}{\frac{\nu}{d}\alpha_h(s_e)} + \sum_{i=1}^m{t_it_i'\rho(s_e)}\\
&= \sum_{h \in G}{\frac{\nu}{d}s_h} + \sum_{i=1}^m{t_it_i'\rho(s_e)}.
\end{split}
\end{align*}
\noindent Here $t_i'\rho(s_e)$ is in $\homset_{\mathcal{E}(A)}(\rho, {\rho \otimes \rho}) = \text{span}\{t_1, \dots, t_m\}$. Repeating the same process but by multiplying on the left by $\rho(s_e')$ and using $\rho(s_e')s_e = \nu/d$ gives us
\begin{align*}
\begin{split}
\rho(s_e') = \sum_{h \in G}{\frac{\nu}{d}s_h'} + \sum_{i=1}^m{\rho(s_e')t_it_i'},
\end{split}
\end{align*}
\noindent where $\rho(s_e')t_i \in \homset_{\mathcal{E}(A)}({\rho \otimes \rho}, \rho) = \text{span}\{t_1', \dots, t_m'\}$. This completes the proof.
\end{proof}\\

\noindent Recall that $\alpha_g(s_h) = \id_{\alpha_g} \otimes (\id_{\alpha_h} \otimes s_e) = \id_{\alpha_{gh}} \otimes s_e = s_{gh}$, and similarly for $\alpha_g(s_h')$. In other words, $\alpha_g(s_h) \in \mathcal{L}$ for all $g, h \in G$. All that remains is to show how $\rho$ and $\alpha_g$ act on our $t_i$ and $t_j'$.\\
...
\noindent\begin{proof} Suppose we define the projections
\begin{align*}
\begin{split}
S \coloneqq \sum_{h \in G}{s_hs_h'}\qquad\text{and}\qquad T \coloneqq \sum_{i=1}^m{t_it_i'},
\end{split}
\end{align*}
\noindent where we recall that $S + T = 1$. %We will determine $S\rho(x)$ and $T\rho(x)$ independently. Well,
%\begin{align*}
%\begin{split}
...
\end{split}
\end{align*}
\noindent that is, $l_g = \alpha_g \circ l_e$ and $\alpha_g \circ l = l$. We claim that $l_e = \varphi$.
\end{proof}\\

\noindent\textcolor{red}{Current mysteries: how is there an element $V(g) \in \homset(\rho^2, \rho^2)$ such that $\alpha_g(T) = V(g)T$ for all $T \in \mathcal{K}$? Answer: I think viewing it as a matrix solves this problem. How can I adapt the proof for $l_e = j_2 \circ j_1^{-1}$ to my setting? Is it even possible? Probably I would need to compute $\rho(\varphi^{-1}(T))$ or something, but this seems quite hard. Should I define $j_1$ and $j_2$? I guess I'll see what I can do. Is $U_\mathcal{K}$ a representation on $\text{End}(\homset_{\mathcal{E}(A)}(\rho, {\rho \otimes \rho}))$? Answer: yes, I think it must be by restriction to $\mathcal{K}$.}
