...
Bbecause the group-like elements of $\mathcal{C}$ have Frobenius--Perron dimension $1$, the dimension of $\rho$ must satisfy $d^2 = md + n$. In other words,
\begin{align*}
\begin{split}
d \coloneqq \frac{m \pm \csqrt{m^2 + 4n}}{2}
\end{split}
\end{align*}
\noindent will be the dimension of $\rho$. Duals of simple objects are simple, so our fusion rules tell us that we must have $[\rho^*] = [\rho]$. Suppose we choose a representative $\rho$ for which $\rho^* = \rho$, as well as representatives $\alpha_g$ such that $\alpha_g \circ \rho = \rho$ for all $g \in G$. We denote by $\nu \in \{-1, 1\}$ the Frobenius--Schur indicator of $\rho$.\\

\noindent With these choices, we will then not necessarily have ``$\rho \circ \alpha_g = \rho$''; by definition, we may only expect $\rho(\alpha_g(x)) = U(g)\rho(x)U(g)^{-1}$ for some invertible $U(g) \in A$. By \hyperref[CommutingInvertibles]{Proposition \ref*{CommutingInvertibles}}, however, we will always have $\alpha_g \circ \alpha_h = \alpha_{gh}$ for all $g, h \in G$. The third equation from our fusion rules above tells us, by definition of the direct sum, that we must have $s_g, s_g', t_i, t_i' \in A$ such that %for $i \in \{1, \dots, m\}$ and $g \in G$ such that
\begin{align*}
\begin{split}
...
\begin{align*}
\begin{split}
s_e \coloneqq \frac{1}{\csqrt{d}}\coev_\rho\qquad\text{and}\qquad s_e' \coloneqq \frac{\nu}{\csqrt{d}}\ev_\rho.
\end{split}
\end{align*}
\noindent This will force $\rho(s_e')s_e = \nu/d = s_e'\rho(s_e)$. Suppose now that $s_g \coloneqq \alpha_g(s_e)$. Recalling the definition of the monoidal product of morphisms in our category and our assumption that $\alpha_g \otimes \rho = \rho$,% this means that
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent for $s_g$. We will now find, for each $g \in G$, isomorphisms $U(g) \in \homset_{\mathcal{E}(A)}({\rho \otimes \rho}, {\rho \otimes \rho})$ satisfying the normalization condition $U(g)s_e = s_e$. Because $\alpha_g \otimes \rho = \rho$ and the spaces $\homset_{\mathcal{E}(A)}(\rho, {\rho \otimes \rho})$ and $\homset_{\mathcal{E}(A)}(\rho, \rho \otimes \alpha_g)$ are $1$-dimensional, there is a unique morphism $U_g \in \homset_{\mathcal{E}(A)}(\rho, \rho \otimes \alpha_g)$ satisfying
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent together with the fact that $\homset_{\mathcal{E}(A)}(\rho, \rho \otimes \alpha_{gh})$ is $1$-dimensional. Thus the map $U : g \mapsto U_g \otimes \id_{\rho}$ defines a representation $U : G \to \homset_{\mathcal{E}(A)}({\rho \otimes \rho}, {\rho \otimes \rho})$. In other words, $U(g)$ is invertible with $U(g)^{-1} = U(g^{-1})$. Since $U_g \otimes \id_\rho = U_g$ as elements of $A$, we have that $\rho(\alpha_g(x)) = U(g)\rho(x)U(g)^{-1}$.\\
...
\end{split}
\end{align*}
\noindent for \textcolor{red}{$\chi_h : G \to \mathbbm{k}^\times$} and $U_\mathcal{K}(g) \in \text{span}\{t_it_j'\}_{i,j = 1}^m = \mathcal{K}\mathcal{K}'$. Note then that $U(g)s_e = s_e$ is equivalent to $\chi_e(g) = 1$. Moreover, $\mathcal{K}\mathcal{K}' = \text{End}(\mathcal{K})$. Certainly $\mathcal{K}\mathcal{K}' \subseteq \text{End}(\mathcal{K})$, since $t_it_j'\mathcal{K} \subseteq \mathcal{K}$; the converse direction comes by comparing dimensions. Thus $U_\mathcal{K} : G \to \text{End}(\mathcal{K})$ defines a representation as the projection of $U(g)$ onto $\mathcal{K}\mathcal{K}'$.\\ %\textcolor{red}{We will later see that these $\chi_h$ are precisely the characters of $G$.}\\
...
\end{split}
\end{align*}
\noindent where $\rho(s_e')t_i \in \mathcal{K}'$ and hence $\rho(s_e')T \in (\mathcal{K}')^2$. The final equation follows directly from $\rho(s_e')\rho(s_e) = 1$. This completes the proof.
\end{proof}\\
...
\begin{align*}
\begin{split}
\varphi^3(x) = \nu x,\qquad\varphi(\alpha_g(\varphi^{-1}(x))) = U_\mathcal{K}(g)x\qquad\text{and}\qquad \textcolor{red}{\varphi^{-1}(\alpha_g(\varphi(x))) = \rho(U(g)) x U(g)^{-1}}
\end{split}
\end{align*}
...
&= \frac{d}{\nu}U(g)\rho^2(s_e') x s_e = \frac{d}{\nu}U(g) x \rho(s_e')s_e = U(g)x = U_\mathcal{K}(g)x. %\frac{d^2}{\nu^2}\rho^2(s_e')\rho(\alpha_g(s_e'))\rho^2(x)\rho^2(s_e)s_e\\
% &= \frac{d^2}{\nu^2}\rho^2(s_e')U(g)\rho(s_e')U(g)^{-1}s_exs_e = \frac{d^2}{\nu^2}\rho^2(s_e')U(g)\rho(s_e')s_exs_e\\
% &= \frac{d}{\nu}\rho^2(s_e')U(g)xs_e = \frac{d}{\nu}U(g)\rho^2(s_e')xs_e = U(g)x = U_\mathcal{K}(g)x.
\end{split}
\end{align*}
\noindent\textcolor{red}{Finally, using the normalization relation $s_e'U(g) = s_e'$, we have}
\begin{align*}
\begin{split}
...
% &= 
\textcolor{red}{\varphi^{-1}(\alpha_g(\varphi(x)))} &\textcolor{red}{= \frac{d^2}{\nu^2}s_e'\rho(\alpha_g(\rho^2(s_e')\rho(x) s_e))\rho(s_e) = \frac{d^2}{\nu^2}s_e'U(g)\rho^3(s_e')\rho^2(x)\rho(s_e)U(g)^{-1}\rho(s_e)}\\
&\textcolor{red}{= \frac{d^2}{\nu^2}s_e'\rho^3(s_e')\rho^2(x)\rho(s_e)U(g)^{-1}\rho(s_e) = \frac{d^2}{\nu^2}\rho(s_e')s_e'\rho^2(x)\rho(s_e)U(g)^{-1}\rho(s_e)}\\
&\textcolor{red}{= \frac{d^2}{\nu^2}\rho(s_e')xs_e'\rho(s_e)U(g)^{-1}\rho(s_e) = \frac{d}{\nu}\rho(s_e')xU(g)^{-1}\rho(s_e)}\\
&\textcolor{red}{= \frac{d}{\nu}\rho(s_e')\rho(U(g))xU(g)^{-1}\rho(s_e)}
...
\noindent The result $\varphi(\alpha_g(\varphi^{-1}(x))) = U_\mathcal{K}(g)x$ can be rewritten as $\varphi \circ V(g) = U_\mathcal{K}(g) \circ \varphi$. In other words, the representations $U_\mathcal{K}$ and $V$ are equivalent and intertwined by $\varphi$. This lemma also admits the following ``dual'' that is proved similarly.\\
...
\begin{align*}
\begin{split}
\varphi'^3(y) = \nu y,\qquad\varphi'(\alpha_g(\varphi'^{-1}(y))) = yU_\mathcal{K}(g)^{-1}\qquad\text{and}\qquad \textcolor{red}{\varphi'^{-1}(\alpha_g(\varphi'(y))) = U(g) y\rho(U(g)^{-1})}
\end{split}
\end{align*}
