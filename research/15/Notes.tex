...
\end{split}
\end{align*}
\noindent Note that because $x \in \homset_{\mathcal{E}(A)}(\rho, \rho \otimes \rho)$, we have that $x \in \text{span}\{t_1, \dots, t_m\}$ and hence $s_h'x = 0$ for all $h \in G$ by our initial direct sum properties. In other words, $U(g)x = U_\mathcal{K}(g)x$. A similar process shows us that
\begin{align*}
\begin{split}
...
\noindent We will find it convenient now to denote both $\mathcal{K} \coloneqq \homset_{\mathcal{E}(A)}(\rho, \rho \otimes \rho) = \text{span}_\mathbbm{k}\{t_1, \dots, t_m\}$ and $\mathcal{K}^{-1} \coloneqq \homset_{\mathcal{E}(A)}(\rho \otimes \rho, \rho) = \text{span}_\mathbbm{k}\{t_1', \dots, t_m'\}$, and let
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent for $p, q \in \mathbb{N}$. In other words, $\mathcal{K}^p\mathcal{K}^{-q} \cong \mathcal{K}^{\otimes p} \otimes (\mathcal{K}^{-1})^{\otimes q}$. We will also define the projections
\begin{align*}
\begin{split}
