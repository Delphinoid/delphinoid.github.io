...
\end{split}
\end{align*}
\noindent for all $g, h \in G$ and $i, j \in \{1, \dots, m\}$. However, there are in general no canonical choices for these elements. Our choices for $s_e \in \homset_{\mathcal{E}(A)}(\id, \rho \otimes \rho)$ and $s_e' \in \homset_{\mathcal{E}(A)}(\rho \otimes \rho, \id)$ %, $t_i \in \homset_{\mathcal{E}(A)}(\rho, \rho \otimes \rho)$ and $t_i' \in \homset_{\mathcal{E}(A)}(\rho \otimes \rho, \rho)$
will be unimportant, but we will make a special choice for each $s_g$. In particular, suppose $s_g \coloneqq \alpha_g(s_e)$. Recalling the definition of the monoidal product of morphisms in our category and our assumption that $\alpha_g \otimes \rho = \rho$,% this means that
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent for $s_g$. We will now find, for each $g \in G$, isomorphisms $U_g \in \homset_{\mathcal{E}(A)}(\rho, \rho \otimes \alpha_g)$ satisfying the normalization condition $U_g s_e = s_e$. Because $\alpha_g \otimes \rho = \rho$ and the spaces $\homset_{\mathcal{E}(A)}(\rho, \rho \otimes \rho)$ and $\homset_{\mathcal{E}(A)}(\rho, \rho \otimes \alpha_g)$ are $1$-dimensional, there exists a unique morphism $U_g$ satisfying
\begin{align*}
\begin{split}
s_e = (U_g \otimes \id_\rho) \circ s_e.
\end{split}
\end{align*}
...
\end{split}
\end{align*}
\noindent together with the fact that $\homset_{\mathcal{E}(A)}(\rho, \rho \otimes \alpha_{gh})$ is $1$-dimensional. Thus the map $U : g \mapsto U_g \otimes \id_{\rho}$ defines a representation $U : G \to \homset_{\mathcal{E}(A)}(\rho \otimes \rho, \rho \otimes \rho)$. In other words, $U(g)$ is invertible with $U(g)^{-1} = U(g^{-1})$, whence $\rho(\alpha_g(x)) = U(g)^{-1}\rho(x)U(g)$.\newpage

\noindent Let $\{t_1, \dots, t_m\}$ be a basis for $\homset_{\mathcal{E}(A)}(\rho, \rho \otimes \rho)$ and $\{t_1', \dots, t_m'\}$ a basis for $\homset_{\mathcal{E}(A)}(\rho \otimes \rho, \rho)$ satisfying $t_i't_j = \delta_{i,j}$. Then $\homset_{\mathcal{E}(A)}(\rho \otimes \rho, \rho \otimes \rho)$ is spanned by $\{t_it_j'\}_{i,j = 1}^m \sqcup \{s_gs_g'\}_{g \in G}$. Because morphisms in $\mathcal{E}(A)$ are nothing but elements of the algebra $A$, we have that
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent In other words, $\homset_{\mathcal{E}(A)}(\rho, \rho \otimes \alpha_g)$ is also spanned by $\{t_it_j'\}_{i,j = 1}^m \sqcup \{s_gs_g'\}_{g \in G}$. It follows that
\begin{align*}
\begin{split}
U_g = \sum_{h \in G}{\chi_h(g)s_hs_h'} + U_\mathcal{K}(g)
\end{split}
\end{align*}
\noindent for $\chi_h : G \to \mathbbm{k}$ and $U_\mathcal{K}(g) \in \text{span}\{t_it_j'\}_{i,j = 1}^m$.\\
...
\begin{align*}
\begin{split}
\varphi : t &\mapsto (\lambda_\rho \otimes \id_{\rho^*}) \circ (\ev_\rho \otimes \id_\rho \otimes \id_{\rho^*}) \circ (\id_{\rho^*} \otimes t \otimes \id_{\rho^*}) \circ (\id_{\rho^*} \otimes \coev_\rho) \circ \rho_{\rho^*}^{-1},
\end{split}
\end{align*}
\noindent with inverse
\begin{align*}
\begin{split}
\varphi^{-1} : t &\mapsto (\id_\rho \otimes \rho_\rho) \circ (\id_\rho \otimes \id_\rho \otimes \ev_\rho) \circ (\id_\rho \otimes t \otimes \id_\rho) \circ (\coev_\rho \otimes \id_\rho) \circ \lambda_\rho^{-1}.
\end{split}
\end{align*}
\noindent Because duals of simple objects are simple, our fusion rules tell us that we must have $\rho^* \cong \rho$. Let's choose $\rho^* = \rho$. Suppose we then let $V(g) : t \mapsto \id_{\alpha_g} \otimes t$ for all $t \in \homset_{\mathcal{E}(A)}(\rho \otimes \rho, \rho)$, which defines another representation $V : G \to \text{End}(\homset_{\mathcal{E}(A)}(\rho \otimes \rho, \rho))$. We observe that
\begin{align*}
\begin{split}
[\varphi^{-1} \circ V(g) \circ \varphi](t) &=
%(\id_{\alpha_g} \otimes \lambda_\rho \otimes \id_\rho) \circ (\id_{\alpha_g} \otimes \ev_\rho \otimes \id_\rho \otimes \id_\rho) \circ (\id_\rho \otimes t \otimes \id_\rho) \circ (\id_\rho \otimes \coev_\rho) \circ (\id_{\alpha_g} \otimes \rho_\rho^{-1})
%U_\mathcal{K}(g).
\end{split}
\end{align*}
%\begin{align*}
