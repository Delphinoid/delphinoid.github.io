...
\end{split}
\end{align*}
\noindent for all $g, h \in G$ and $i, j \in \{1, \dots, m\}$. However, there are in general no canonical choices for these elements. We will make the following choices. First, because the group-like elements of $\mathcal{C}$ have Frobenius--Perron dimension $1$, the dimension of $\rho$ must satisfy $d^2 = md + n$. In other words,
\begin{align*}
\begin{split}
d \coloneqq \frac{m \pm \csqrt{m^2 + 4n}}{2}
\end{split}
\end{align*}
\noindent will be the dimension of $\rho$. Because duals of simple objects are simple, our fusion rules tell us that we must have $\rho^* \cong \rho$. Let's choose $\rho^* = \rho$ and denote by $\nu \in \{-1, 1\}$ the Frobenius--Schur indicator of $\rho$. Since $s_e \in \homset_{\mathcal{E}(A)}(\id, {\rho \otimes \rho})$ and $s_e' \in \homset_{\mathcal{E}(A)}({\rho \otimes \rho}, \id)$ live in $1$-dimensional spaces, we can choose them to be scalar multiples of the coevaluator and evaluator, normalized with respect to the direct sum condition $s_e's_e = 1$. That is,
\begin{align*}
\begin{split}
...
\noindent Let $\{t_1, \dots, t_m\}$ be a basis for $\mathcal{K}$ and $\{t_1', \dots, t_m'\}$ a basis for $\mathcal{K}'$ satisfying $t_i't_j = \delta_{i,j}$, which we know must exist by our direct sum properties. Every map in $\homset_{\mathcal{E}(A)}({\rho \otimes \rho}, {\rho \otimes \rho})$ is a linear combination of maps that go from ${\rho \otimes \rho} \to X \to {\rho \otimes \rho}$, for some simple object $X$, whence it follows that $\homset_{\mathcal{E}(A)}({\rho \otimes \rho}, {\rho \otimes \rho})$ is spanned by $\{t_it_j'\}_{i,j = 1}^m \sqcup \{s_gs_g'\}_{g \in G}$. %Because morphisms in $\mathcal{E}(A)$ are nothing but elements of the algebra $A$, we have that
%\begin{align*}
%\begin{split}
...
\end{split}
\end{align*}
\noindent for $\chi_h : G \to \mathbb{T}$ and $U_\mathcal{K}(g) \in \text{span}\{t_it_j'\}_{i,j = 1}^m = \mathcal{K}\mathcal{K}'$. In particular, $\mathcal{K}\mathcal{K}' = \text{End}(\mathcal{K})$. Certainly $\mathcal{K}\mathcal{K}' \subseteq \text{End}(\mathcal{K})$, since $t_it_j'\mathcal{K} \subseteq \mathcal{K}$; the converse direction comes by comparing dimensions. Thus $U_\mathcal{K} : G \to \text{End}(\mathcal{K})$ defines a representation as the projection of $U(g)$ onto $\mathcal{K}\mathcal{K}'$. %\textcolor{red}{We will later see that these $\chi_h$ are precisely the characters of $G$.}\\
Finally, for each $g \in G$, we have an element $V(g) : x \mapsto \alpha_g(x)$, defining yet another representation $V : G \to \text{End}(\mathcal{K})$.\\
...
\end{split}
\end{align*}
\noindent where $\rho(s_e')t_i \in \mathcal{K}'$ and hence $\rho(s_e')T \in (\mathcal{K}')^2$. This completes the proof.
\end{proof}\\
...
\noindent\begin{lemma} The map $\varphi : \mathcal{K} \to \mathcal{K}$ given by
\begin{align*}
\begin{split}
...
&= \frac{d^2}{\nu^2}U(g)\rho^2(s_e')\rho(s_e')\rho^2(x)\rho^2(s_e)s_e = U(g)x = U_\mathcal{K}(g)x. %\frac{d^2}{\nu^2}\rho^2(s_e')\rho(\alpha_g(s_e'))\rho^2(x)\rho^2(s_e)s_e\\
...
\end{split}
\end{align*}
\noindent\textcolor{red}{Finally, using the normalization relation $U(g)s_e = s_e$, we have}
\begin{align*}
\begin{split}
...
\noindent\begin{lemma} There exists a unique linear map $l : \mathcal{K} \to \mathcal{K}^2\mathcal{K}'$ such that
\begin{align*}
\begin{split}
...
\noindent\textcolor{red}{To-do: do all of this for $\mathcal{K}'$.}\\

\noindent We have thus shown that $\rho$ and $\alpha_g$ are endomorphisms of the Leavitt algebra, as they are given by noncommutative polynomials in our Leavitt algebra generators.
