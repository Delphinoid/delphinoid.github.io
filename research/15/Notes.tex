...
\end{split}
\end{align*}
\noindent for $s_g$. We will now find, for each $g \in G$, an isomorphism $U(g) \in \homset_{\mathcal{E}(A)}(\rho, \rho \otimes \alpha_g)$. We will find one satisfying the normalization condition $U(g)s_e = s_e$. Because $\alpha_g \otimes \rho = \rho$ and the space $\homset_{\mathcal{E}(A)}(\id, \rho \otimes \rho)$ containing $s_e$ is $1$-dimensional, there certainly exists a morphism $U(g)$ satisfying
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent \textcolor{red}{via abstract nonsense}, whence $g \mapsto U(g) \otimes \id_{\rho}$ gives a representation of $G$ on $\homset_{\mathcal{E}(A)}(\rho \otimes \rho, \rho \otimes \rho)$. In other words, $U(g)$ is invertible with $U(g)^{-1} = U(g^{-1})$, whence $\rho(\alpha_g(x)) = U(g)^{-1}\rho(x)U(g)$.\\

\noindent We now wish to show that $\rho$ and $\alpha_g$ restrict to endomorphisms of the Leavitt algebra $\mathcal{L} \coloneqq \mathcal{L}_{m+n}$, with generators $s_g, s_g', t_i, t_i'$ for $g \in G$ and $i \in \{1, \dots, m\}$.\\
