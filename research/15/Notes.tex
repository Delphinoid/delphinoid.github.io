...
\end{split}
\end{align*}
\noindent for $s_g$. We will now find, for each $g \in G$, isomorphisms $U(g) \in \homset_{\mathcal{E}(A)}({\rho \otimes \rho}, {\rho \otimes \rho})$ satisfying the normalization condition $U(g)s_e = s_e$. Because $\alpha_g \otimes \rho = \rho$, the space $\homset_{\mathcal{E}(A)}(\rho, \rho \otimes \alpha_g)$ is $1$-dimensional, whence there must exist a unique scalar $U_g \in \homset_{\mathcal{E}(A)}(\rho, \rho \otimes \alpha_g)$ satisfying
\begin{align*}
\begin{split}
...
\begin{align*}
\begin{split}
\sum_{i=1}^m \rho(s_e')t_it_i'\rho(s_e) = \frac{m}{d}.
\end{split}
\end{align*}
...
\end{split}
\end{align*}
\noindent where $\rho(s_e')t_i \in \mathcal{K}'$ and hence $\rho(s_e')T \in (\mathcal{K}')^2$. The final equation follows directly from $\rho(s_e')\rho(s_e) = 1$.
...
\noindent and moreover we have
\begin{align*}
\begin{split}
\varphi^3(x) = \nu x,\qquad\varphi(\alpha_g(\varphi^{-1}(x))) = U_\mathcal{K}(g)x\qquad\text{and}\qquad\varphi^{-1}(\alpha_g(\varphi(x))) = \rho(U(g)) x U(g)^{-1}
\end{split}
\end{align*}
...
%\end{split}
%\end{align*}
\noindent The final result follows. This completes the proof.
\end{proof}\\
...
\begin{align*}
\begin{split}
\varphi'^3(y) = \nu y,\qquad\varphi'(\alpha_g(\varphi'^{-1}(y))) = yU_\mathcal{K}(g)^{-1}\qquad\text{and}\qquad\varphi'^{-1}(\alpha_g(\varphi'(y))) = U(g) y\rho(U(g)^{-1})
\end{split}
\end{align*}
...
\end{enumerate}
\noindent Note that we only assume that $\mathcal{C}$ is pivotal so that $\nu\varphi$ is order $3$. \textcolor{red}{How do these choices affect things?}\\

\noindent Recall that the Leavitt algebra $\mathcal{L}$ admits an involutive antiautomorphism $-' : \mathcal{L} \to \mathcal{L}$ that exchanges each $s_g$ and $t_i$ with their primed versions and vice versa. Note that $\rho$, $\alpha_g$ and $\varphi$ are compatible with this map; in particular, $\rho(-)' = \rho(-')$, $\alpha_g(-)' = \alpha_g(-')$ and $\varphi(-)' = \varphi'(-')$. We shall henceforth define $\varphi$ and $l$ on $\mathcal{K}'$ via $\varphi'$ and $l'$, respectively.\\

\noindent Our setting diverges from the unitary setting here due to the fact that $U(g)'$ is not necessarily equal to $U(g)^{-1}$, since $\overline{\chi_h(g)}$ is not necessarily equal to $\chi_h(g)^{-1}$ and $U_\mathcal{K}(g)'$ is not necessarily equal to $U_\mathcal{K}(g)^{-1}$. In fact, $\chi_h(g)$ is precisely where we will start to see issues.\\
...
\noindent\begin{corollary} The map $\chi$ is a symmetric bicharacter. In particular,
\begin{gather*}
\textcolor{red}{\chi_{h}(g) = \chi_g(h)},\\
...
\end{split}
\end{align*}
\noindent showing that $\alpha_h(U(g)) = \chi_{h^{-1}}(g)U(g)$. %Moreover,
%\begin{align*}
%\begin{split}
%\alpha_h(U(g)) &= \sum_{k \in G}{\chi_{h^{-1}k}(g)s_ks_k'} + \alpha_h(U_\mathcal{K}(g))\\
% &= \sum_{k \in G}{\chi_{h^{-1}k}(g)s_ks_k'} + V(h)U_\mathcal{K}(g)V(h)^{-1}\\
% &= \sum_{k \in G}{\chi_{h^{-1}k}(g)s_ks_k'} + \chi_{h}(g)^{-1}U_\mathcal{K}(g),
%\end{split}
%\end{align*}
%\noindent whence $\chi_{h^{-1}}(g) = \chi_{h}(g)^{-1}$.
Suppose we now define a map
\begin{align*}
\begin{split}
\eta_{h,g} : z \mapsto \frac{d}{\nu}\rho(\alpha_h(s_e'))\rho(z)\alpha_g(s_e),
\end{split}
\end{align*}
\noindent for all $z \in \homset_{\mathcal{E}(A)}(\rho \otimes \alpha_{g^{-1}}, \alpha_h \otimes \rho)$. Note that $\alpha_h(U(g^{-1})^{-1}), U(g^{-1})^{-1} \in \homset_{\mathcal{E}(A)}(\rho \otimes \alpha_{g^{-1}}, \alpha_h \otimes \rho)$. Applying $\eta_{h,g}$, we obtain
%\begin{align*}
%\begin{split}
%\eta_{h,g}(\alpha_h(U(g))) &= \frac{d}{\nu}\rho(\alpha_h(s_e'))\rho(\alpha_h(U(g)))\alpha_g(s_e)\\
% &= \frac{d}{\nu}\rho(\alpha_h(s_e'U(g)))\alpha_g(s_e)\\
% &= \frac{d}{\nu}\rho(\alpha_h(s_e'))\alpha_g(s_e)\\
% &= \frac{d}{\nu}U(h)\rho(s_e')U(h^{-1})\alpha_g(s_e)\\
% &= \frac{d}{\nu}\chi_g(h^{-1})U(h)\rho(s_e')\alpha_g(s_e)\\
% &= \frac{d}{\nu}\chi_g(h^{-1})U(h)\alpha_g(\rho(s_e')s_e)\\
% &= \chi_g(h^{-1})U(h).
%\end{split}
%\end{align*}
%\noindent Similarly,
\begin{align*}
\begin{split}
\eta_{h,g}(U(g)) &= \frac{d}{\nu}\rho(\alpha_h(s_e'))\rho(U(g))\alpha_g(s_e)\\
&= \frac{d}{\nu}\rho(\alpha_h(s_e')U(g))\alpha_g(s_e)\\
&= \frac{d}{\nu}\chi_{h}(g)\rho(\alpha_h(s_e'))\alpha_g(s_e)\\
&= \frac{d}{\nu}\chi_{h}(g)U(h)\rho(s_e')U(h^{-1})\alpha_g(s_e)\\
&= \frac{d}{\nu}\chi_{h}(g)\chi_g(h^{-1})U(h)\rho(s_e')\alpha_g(s_e)\\
&= \chi_{h}(g)\chi_g(h)^{-1}U(h).\\
\end{split}
\end{align*}
\noindent\textcolor{red}{Supposedly the diagrammatics imply that $\eta_{h,g}(U(g)) = U(h)$ and hence that $\chi_h(g) = \chi_g(h)$.} This completes the proof.
\end{proof}\\
...
\end{split}
\end{align*}
\noindent Now, recall that the left quantum trace of $U_\mathcal{K}(g)$ as an element of $\homset_{\mathcal{E}(A)}({\rho \otimes \rho}, {\rho \otimes \rho})$ is given by
\begin{align*}
\begin{split}
\textup{Tr}^L(U_\mathcal{K}(g)) \coloneqq \textup{ev}_\rho\rho(\textup{ev}_\rho)U_\mathcal{K}(g)\rho(\textup{coev}_\rho)\textup{coev}_\rho = d^2 s_e'\rho(s_e')U_\mathcal{K}(g)\rho(s_e)s_e,
\end{split}
\end{align*}
\noindent and similarly for $\textup{Tr}^L(U(g))$. Moreover, $\rho(s_e')U_\mathcal{K}(g)\rho(s_e) \in \mathbbm{k}$, as it is in the $1$-dimensional space $\homset_{\mathcal{E}(A)}(\rho, \rho)$. Thus, because $s_e's_e = 1$, it follows that
\begin{align*}
\begin{split}
\delta_{g,e} = \frac{1}{d^2}\textup{Tr}^L(U(g)) = \frac{1}{d^2}\sum_{h \in G} \chi_h(g) + \frac{1}{d^2}\textup{Tr}^L(U_\mathcal{K}(g)).
\end{split}
\end{align*}
\noindent Observe that when $g = e$, multiplying through by $d^2$ recovers
\begin{align*}
\begin{split}
d^2 = \abs{G} + \textup{Tr}^L(T) = n + md
\end{split}
\end{align*}
\noindent by \hyperref[RhoExpansion]{Lemma \ref*{RhoExpansion}}.
\end{proof}\\
