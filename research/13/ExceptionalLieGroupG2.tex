...
\begin{align*}
\begin{split}
\alpha_1 \coloneqq \lambda_2,\qquad \alpha_2 \coloneqq \lambda_1 - \lambda_2
\end{split}
\end{align*}
\noindent with highest root
\begin{align*}
\begin{split}
\mu \coloneqq 3\alpha_1 + 2\alpha_2.\\[\linespacing]
\end{split}
\end{align*}
\end{corollary}
...
\begin{alignat*}{3}
\lambda_2 &= \alpha_1,\qquad&\lambda_1 - \lambda_3 &= 3\alpha_1 + 2\alpha_2,\qquad&\lambda_2 - \lambda_3 &= 3\alpha_1 + \alpha_2,\\
\lambda_1 &= \alpha_1 + \alpha_2,\qquad&\lambda_1 - \lambda_2 &= \alpha_2,\qquad&-\lambda_3 &= 2\alpha_1 + \alpha_2,
\end{alignat*}
...
\begin{align*}
\begin{split}
A \coloneqq \begin{pmatrix}\hphantom{-}2&-3\\-1&\hphantom{-}2\end{pmatrix}.\\[\linespacing]
\end{split}
\end{align*}
\end{theorem}
...
\begin{align*}
\begin{split}
H_{\alpha_1} = \frac{1}{24}iG_{23} - \frac{1}{12}iG_{45} + \frac{1}{24}iG_{67}\qquad\text{and}\qquad H_{\alpha_2} = -\frac{1}{8}iG_{23} + \frac{1}{8}iG_{45}.
\end{split}
\end{align*}
\noindent We therefore have
\begin{align*}
\begin{split}
(\alpha_1, \alpha_1) &= B(H_{\alpha_1}, H_{\alpha_1}) = 8\left(\left(-\frac{1}{24}\right)^2 + \left(\frac{1}{12}\right)^2 + \left(-\frac{1}{24}\right)^2\right) = \frac{1}{12},\\
(\alpha_2, \alpha_2) &= B(H_{\alpha_2}, H_{\alpha_2}) = 8\left(\left(\frac{1}{8}\right)^2 + \left(-\frac{1}{8}\right)^2\right) = \frac{1}{4},\\
(\alpha_1, \alpha_2) &= B(H_{\alpha_1}, H_{\alpha_2}) = 8\left(\left(-\frac{1}{24}\right)\left(\frac{1}{8}\right) + \left(\frac{1}{12}\right)\left(-\frac{1}{8}\right)\right) = -\frac{1}{8}.
\end{split}
\end{align*}
...
\noindent Note that $(\alpha_1, \alpha_1) = 1/12$ tells us that $\abs{\alpha_1} = 1/\csqrt{12}$, and similarly $\abs{\alpha_2} = 1/2$. Meanwhile, from $(\alpha_1, \alpha_2) = -1/8$ we may deduce that the angle between $\alpha_1$ and $\alpha_2$ is
\begin{align*}
\begin{split}
\theta = \cos^{-1}\left(-\frac{\abs{\alpha_1}\abs{\alpha_2}}{8}\right) = \cos^{-1}\left(-\frac{\csqrt{3}}{2}\right) = \frac{5\pi}{6}.
\end{split}
\end{align*}
\noindent Thus we can draw our root system as follows.
\begin{center}
...
\draw[-latex] (0,0) -- (-1/2,0);
\node at (1/2+0.05,0) {$\alpha_2$};
%
\draw[-latex, trig format=rad] (0,0) -- ({cos(5*pi/6)/sqrt(12)},{sin(5*pi/6)/sqrt(12)});
\draw[-latex, trig format=rad] (0,0) -- ({-cos(5*pi/6)/sqrt(12)},{-sin(5*pi/6)/sqrt(12)});
\node[trig format=rad] at ({cos(5*pi/6)*(1/sqrt(12)+0.05)},{sin(5*pi/6)*(1/sqrt(12)+0.05)}) {$\alpha_1$};
%
\draw[-latex, trig format=rad] (0,0) -- ({cos(pi/6)/sqrt(12)},{sin(pi/6)/sqrt(12)});
\draw[-latex, trig format=rad] (0,0) -- ({-cos(pi/6)/sqrt(12)},{-sin(pi/6)/sqrt(12)});
\node[trig format=rad] at ({cos(pi/6)*(1/sqrt(12)+0.075)},{sin(pi/6)*(1/sqrt(12)+0.075)}) {$\alpha_1 + \alpha_2$};
%
\draw[-latex, trig format=rad] (0,0) -- ({0},{2*sin(5*pi/6)/sqrt(12)});
\draw[-latex, trig format=rad] (0,0) -- ({0},{-2*sin(5*pi/6)/sqrt(12)});
\node[trig format=rad] at ({0},{sin(5*pi/6)*(2/sqrt(12)+0.075)}) {$2\alpha_1 + \alpha_2$};
%
\draw[-latex, trig format=rad] (0,0) -- ({cos(4*pi/6)/2},{sin(2*pi/6)/2});
\draw[-latex, trig format=rad] (0,0) -- ({-cos(4*pi/6)/2},{-sin(2*pi/6)/2});
\node[trig format=rad] at ({cos(4*pi/6)*(1/2+0.05)},{sin(2*pi/6)*(1/2+0.05)}) {$3\alpha_1 + \alpha_2$};
%
\draw[-latex, trig format=rad] (0,0) -- ({cos(2*pi/6)/2},{sin(2*pi/6)/2});
\draw[-latex, trig format=rad] (0,0) -- ({-cos(2*pi/6)/2},{-sin(2*pi/6)/2});
\node[trig format=rad] at ({cos(2*pi/6)*(1/2+0.05)},{sin(2*pi/6)*(1/2+0.05)}) {$3\alpha_1 + 2\alpha_2$};
%
...
\end{tikzpicture}
\end{center}
\noindent This diagram immediately tells us that the Weyl group for $G_2$ is the dihedral group of order $12$,
\begin{align*}
\begin{split}
W = D_6 \coloneqq \langle s, t : s^2 = t^2 = (st)^6 = 1\rangle.
\end{split}
\end{align*}
\noindent In particular, the simple reflections are given by
\begin{align*}
\begin{split}
s_i(v) \coloneqq v - 2\frac{(v, \alpha_i)}{(\alpha_i, \alpha_i)}\alpha_i,
\end{split}
\end{align*}
\noindent for $i \in \{1, 2\}$; this is nothing but the reflection across the line perpendicular to $\alpha_i$. Here $s \coloneqq s_1$ is the reflection across $3\alpha_1 + 2\alpha_2$, while $t \coloneqq s_2$ is the reflection across $2\alpha_1 + \alpha_2$. The {\em fundamental Weyl chamber} is by definition $\{v \in \mathbb{R}^2 : (v, \alpha_1) > 0, (v, \alpha_2) > 0\}$; this region has been shaded in the diagram above. For $G_2$, the fundamental Weyl chamber is the same as the {\em fundamental Weyl alcove} $\{v \in \mathbb{R}^2 : (v, \alpha_1) > 0, (v, \alpha_2) > 0, (v, \mu) < 1\}$, where we recall that the highest root is $\mu \coloneqq 3\alpha_1+2\alpha_2$. We have also labelled the fundamental alcove by $1$, and the other alcoves by the element of the Weyl group for which they are the image of the fundamental alcove under.\\[\linespacing]
...
\ruledsectionstar{Appendix}{Appendix}
...
\noindent Going one step further, let $K \coloneqq (G_2^\mathbb{C})^\gamma \cong \textup{SL}(2, \mathbb{C}) \times \textup{SL}(2, \mathbb{C})$. This corresponds to a Weyl group $W_K = \mathbb{Z}/2\mathbb{Z} \times \mathbb{Z}/2\mathbb{Z}$. There are three distinct copies of the Klein four-group $\mathbb{Z}/2\mathbb{Z} \times \mathbb{Z}/2\mathbb{Z}$ living inside $D_6$; these are
\begin{align*}
\begin{split}
\langle s, tstst\rangle &= \{1, s, tstst, ststst\},\\
\langle sts, tst\rangle &= \{1, sts, tst, ststst\},\\
\langle t, ststs\rangle &= \{1, t, ststs, ststst\}.
\end{split}
\end{align*}
\noindent The generators we have chosen correspond to orthogonal roots; in particular, $s$ is a reflection across $3\alpha_1+2\alpha_2$, $t$ is a reflection across $2\alpha_1+\alpha_2$, $sts$ is a reflection across $\alpha_1+\alpha_2$, $tst$ is a reflection across $3\alpha_1+\alpha_2$, $ststs$ is a reflection across $\alpha_2$ and $tstst$ is a reflection across $\alpha_1$. We could alternatively have found these candidates by observing that $W_K$ must be generated by orthogonal roots -- as it is given by two commuting copies of $A_1$ -- and these candidates are easily seen to be the only three sets of orthogonal roots. The Atlas of Lie Groups and Representations software is then able to tell us that the generators for $W_K$ correspond to the roots $3\alpha_1+\alpha_2$ and $\alpha_1+\alpha_2$, meaning $W_K = \langle sts, tst\rangle$.
