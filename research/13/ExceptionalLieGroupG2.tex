...
\begin{align*}
\begin{split}
\langle s,\hphantom{ts} (st)^3\rangle &= \{1, ststst, tstst, s\},\\
\langle sts, (st)^3\rangle &= \{1, ststst, sts, tst\},\\
\langle t,\hphantom{ss} (st)^3\rangle &= \{1, ststst, ststs, t\}.
\end{split}
\end{align*}
\noindent However, because $W_K$ is given by two commuting copies of $A_2$, it must be generated by two orthogonal elements. We immediately see that $\langle sts, (st)^3\rangle$ is the unique subgroup satisfying this condition: the roots corresponding to $s$ and $t$ are orthogonal to $3\alpha_1 + 2\alpha_2$ and $2\alpha_1+\alpha_2$, respectively, which certainly do not correspond to $(st)^3$. In fact, arguing via symmetry, we can see that no other option would be viable anyway. Thus we have the set of right cosets $W_K\backslash W = \{W_K, W_Ks, W_Kt\}$.
